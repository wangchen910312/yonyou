        	...
        	MERGE INTO PT_0320I T1
		        USING (
		                SELECT  A.DLR_CD
								...
								,TO_CHAR(SYSDATE - V_DATE, 'YYYYMM') as YYMM
                                ...
                                ,'N' AS BAT_STAT_CD
                                ,SUM(PAR_SALE_QTY) AS PAR_SALE_QTY --销售数量
                                ,SUM(SER_SALE_QTY) AS SER_SALE_QTY --出库数量
                                ,SUM(PAR_SALE_BASE_AMT) AS PAR_SALE_BASE_AMT --销售金额
                                ,SUM(SER_SALE_BASE_AMT) AS SER_SALE_BASE_AMT --出库金额
				           FROM (
						            SELECT   A.DLR_CD
							                ,A.AVLB_STOCK_QTY       
							                ,A.RESV_STOCK_QTY       
							                ...
							                ,NVL(D.SER_SALE_QTY,0) AS SER_SALE_QTY  ---配件维修出库数量
							              	,NVL(D.PAR_SALE_BASE_AMT,0) AS PAR_SALE_BASE_AMT ---配件销售金额
							              	,NVL(D.SER_SALE_BASE_AMT,0) AS SER_SALE_BASE_AMT ---配件出库金额
							                ----20210621--------
	            ...
	            UPDATE 
                   SET    T1.IAN_AVLB_STOCK_QTY = T2.AVLB_STOCK_QTY	
						...
						, T1.PAR_SALE_QTY = T2.PAR_SALE_QTY --销售数量
						, T1.SER_SALE_QTY = T2.SER_SALE_QTY --出库数量
						, T1.PAR_SALE_BASE_AMT = T2.PAR_SALE_BASE_AMT --销售金额
						, T1.SER_SALE_BASE_AMT = T2.SER_SALE_BASE_AMT --出库金额
		        WHEN NOT MATCHED THEN
		            INSERT(
							...
							, T1.IF_REG_DT
							, T1.BAT_STAT_CD
							, T1.PAR_SALE_QTY --销售数量
							, T1.SER_SALE_QTY --出库数量
							, T1.PAR_SALE_BASE_AMT --销售金额
							, T1.SER_SALE_BASE_AMT --出库金额
		            )VALUES(
						  T2.DLR_CD    
						, T2.YYMM	
						, T2.AVLB_STOCK_QTY	
						...
						, T2.IF_REG_DT
						, T2.BAT_STAT_CD
						, T2.PAR_SALE_QTY --销售数量
						, T2.SER_SALE_QTY --出库数量
						, T2.PAR_SALE_BASE_AMT --销售金额
						, T2.SER_SALE_BASE_AMT --出库金额
		            );                              
        EXCEPTION
        	WHEN OTHERS THEN

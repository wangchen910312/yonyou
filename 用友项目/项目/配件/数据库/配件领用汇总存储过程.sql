...
FROM ( 
 select DLR_CD,YEAR_MONTH,DLR_NM,SUM(PART_NUM) AS PART_NUM,SUM(DELIVERY_AMT) AS DELIVERY_AMT,SUM(COST_AMT) AS COST_AMT from PT_0911T t 
where 1=1  AND TO_DATE(YEAR_MONTH,'YYYY-MM')  >=  TO_DATE('2021-05','YYYY-MM') 
AND TO_DATE(YEAR_MONTH,'YYYY-MM')  <= TO_DATE('2021-06','YYYY-MM')  
group by DLR_CD,YEAR_MONTH,DLR_NM
 )T1  WHERE  ROWNUM <= 10 
)   WHERE RNUM > 0;

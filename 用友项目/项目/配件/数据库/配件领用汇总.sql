	...
	select * from CM_0301T t where t.CMM_GRP_CD_NM like '%维修委托类型%';
	
	--查询共同编码值
	select CMM_CD,CMM_CD_NM from CM_0303T t where t.CMM_GRP_CD = 'SER031';
	
	--insert into PT_0911IS (ID,DLR_CD,DLR_NM,DELEGATE_TYPE,PART_NUM,DELIVERY_AMT,COST_AMT,YEAR_MONTH,REMARK,CREATEDATE,UPDATETIME,IFRESULT,IFFAILMSG)	

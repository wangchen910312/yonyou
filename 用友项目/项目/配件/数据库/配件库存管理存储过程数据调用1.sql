SELECT
	...
	,
	--TO_CHAR ('202105', 'YYYYMM') AS YYMM --改成 '202105'
	--,
					...
					(
						...
						FROM
							(
								SELECT
									A .DLR_CD,
									A .PLT_CD,
									A .ITEM_CD --,SUM(NVL(A.AVLB_STOCK_QTY,0)) + SUM(NVL(A.RESV_STOCK_QTY,0)) - SUM(NVL(A.BORROW_QTY,0)) + SUM(NVL(A.RENT_QTY,0)) AS STOCK_QTY   --账面库存
									,
									SUM (
										NVL2 (
											A .ITEM_CD,
											(
												NVL (A .AVLB_STOCK_QTY, 0) + NVL (A .RESV_STOCK_QTY, 0) + NVL (A .CLAM_STOCK_QTY, 0)
											),
											0
										)
									) AS STOCK_QTY --账面库存
						...
						LEFT JOIN (
							SELECT
								...
								SUM (
									NVL (PRE_MM_DDLN_STOCK_AMT, 0)
								) AS PRE_STOCK_AMT,
								SUM (
									(
										NVL (AVLB_STOCK_QTY, 0) + NVL (RESV_STOCK_QTY, 0) - NVL (BORROW_QTY, 0) + NVL (RENT_QTY, 0)
									) * PSNT_MM_DDLN_MOVE_AVG_PRC
								) AS STOCK_AMT
							FROM
								PT_0902T
							WHERE
			...
			FROM
				(
					SELECT
						...
						B.MVT_DOC_LINE_NO,
						B.PLT_CD,
						B.STRGE_CD --update GI_QTY 出库数量 by tianjx  
						,
						CASE
					WHEN A .MVT_TP IN (
						'11',
					...
					WHEN A .MVT_TP IN ('21', '23', '25') THEN
						CASE
					WHEN B.DEBIT_CREDIT_DSTIN_CD = 'H' THEN
						NVL (B.ITEM_QTY, 0)
					ELSE
						0
					END
					WHEN A .MVT_TP IN ('22', '24', '26') THEN
						CASE
					WHEN B.DEBIT_CREDIT_DSTIN_CD = 'H' THEN
						NVL (B.ITEM_QTY, 0)
					ELSE
				...
				,
				CASE
			WHEN A .MVT_TP IN ('61') THEN
				NVL (B.ITEM_QTY, 0)
			WHEN A .MVT_TP IN ('62') THEN
				NVL (B.ITEM_QTY, 0) * - 1
			ELSE
				0
			END OBT_GI_QTY,

...
public class CorpStockConditionVO extends AbstractExcelDataBinder{
    ...
    public void bind(int rowNo, int cellNo, Object value) throws ExcelDataBindingException {
        try{
            switch(cellNo){
                case 0:this.setCarlineCd(ExcelDataBinderUtil.toString(value).trim()); break;
                case 1:this.setModelCd(ExcelDataBinderUtil.toString(value).trim()); break;
                case 2:this.setOcnCd(ExcelDataBinderUtil.toString(value).trim()); break;
                case 3:this.setExtColorCd(ExcelDataBinderUtil.toString(value).trim()); break;
                case 4:this.setIntColorCd(ExcelDataBinderUtil.toString(value).trim()); break;
                case 5:this.setTotStockQty(Integer.parseInt(ExcelDataBinderUtil.toString(value).trim())); break;
                default :
            }
        }catch(Exception e){
    ...
    public List<ExcelUploadError> validate(ExcelDataBindContext context) {
        ...
        // 차관코드
        if(StringUtils.isBlank(this.getModelCd())){

            String msg = messageSource.getMessage(
                "global.info.required.field"
                , new String[]{
                        messageSource.getMessage("global.lbl.modelCode", null, LocaleContextHolder.getLocale())
                }
                , LocaleContextHolder.getLocale()
            );

            errors.add(new ExcelUploadError(context.getRow(), 1, this.getModelCd(), msg));
        }
        ...
        if(StringUtils.isBlank(this.getOcnCd())){
            ...
            errors.add(new ExcelUploadError(context.getRow(), 2, this.getOcnCd(), msg));
        }

        //외색코드
        if(StringUtils.isBlank(this.getExtColorCd())){

            String msg = messageSource.getMessage(
                "global.info.required.field"
                , new String[]{
                        messageSource.getMessage("global.lbl.extColorCd", null, LocaleContextHolder.getLocale())
                }
                , LocaleContextHolder.getLocale()
            );

            errors.add(new ExcelUploadError(context.getRow(), 3, this.getExtColorCd(), msg));
        }

        //내색코드
        if(StringUtils.isBlank(this.getIntColorCd())){
            ...
            errors.add(new ExcelUploadError(context.getRow(), 4, this.getIntColorCd(), msg));
        }
        ...
        if(StringUtils.isBlank(this.getTotStockQty()+"")){
            ...
            errors.add(new ExcelUploadError(context.getRow(), 5, this.getTotStockQty()+"", msg));
        }

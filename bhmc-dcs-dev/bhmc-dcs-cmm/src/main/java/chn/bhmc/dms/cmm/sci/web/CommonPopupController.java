...
public class CommonPopupController extends HController {
    ...
    @ResponseBody
    public SearchResult selectPortlets(@RequestBody PortletTaskMappingSearchVO searchVO) throws Exception {
        SearchResult result = new SearchResult();

        result.setTotal(portletTaskMappingService.selectPortletTaskMappingByConditionCnt(searchVO));
        if(result.getTotal() != 0){
            result.setData(portletTaskMappingService.selectPortletTaskMappingByCondition(searchVO));
        }

...
public class CommonCodeController extends HController {
     ...
     * 공통코드 IF 관리 서비스
     */
    @Resource(name="commonCodeIfService")
    CommonCodeIfService commonCodeIfService;
   ...
   public boolean multiCommonCodeGroups(@Validated @RequestBody CommonCodeGroupSaveVO saveVO) throws Exception {
       ...
       //공통코드 그룹 정보 인터페이스 전송 테이블에 저장
       commonCodeIfService.multiCommonCodeGroups(saveVO);

       //공통코드 그룹 인터페이스 호출
       selectExecuteIf("COM007");
    ...
    public boolean multiCommonCodeGroupTxts(@Validated @RequestBody CommonCodeGroupTxtSaveVO saveVO) throws Exception {
        ...
        //공통코드 그룹 텍스트 정보 인터페이스 전송 테이블에 저장
        commonCodeIfService.multiCommonCodeGroupTxts(saveVO);

        //공통코드 그룹 텍스트 인터페이스 호출
        selectExecuteIf("COM008");
    ...
    public boolean multiCommonCodes(@Validated @RequestBody CommonCodeSaveVO saveVO) throws Exception {
        ...
        //공통코드 정보 인터페이스 전송 테이블에 저장
        commonCodeIfService.multiCommonCodes(saveVO);

        //공통코드 인터페이스 호출
        selectExecuteIf("COM009");
    ...
    public boolean multiCommonCodeTxts(@Validated @RequestBody CommonCodeTxtSaveVO saveVO) throws Exception {
        ...
        //공통코드 텍스트 정보 인터페이스 전송 테이블에 저장
        commonCodeIfService.multiCommonCodeTxts(saveVO);

        //공통코드 텍스트 인터페이스 호출
        selectExecuteIf("COM010");
     ...
     * @throws Exception
     */
    private void selectExecuteIf(String ifId) throws Exception {

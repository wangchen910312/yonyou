...
<section id="callAccessoryMngWindow" class="pop_wrap">
    ...
    <div class="header_area">
        <div class="btn_right">
            ...
            <button type="button" id="btnApplyReq" class="btn_m"><spring:message code='sal.btn.releaseReq' /></button>      <!-- 출고요청 -->
            <button type="button" id="btnApplyCancel" class="btn_m"><spring:message code='sal.btn.releaseCancel' /></button>      <!-- 출고취소 -->
...
$(document).ready(function() {
    ...
    $("#btnSearch").kendoButton({
        click:function(e){
            ...
            if( ($("#sStartGoodsContractDt").data("kendoExtMaskedDatePicker").value() != null && $("#sEndGoodsContractDt").data("kendoExtMaskedDatePicker").value() == null)
                ||($("#sEndGoodsContractDt").data("kendoExtMaskedDatePicker").value() != null && $("#sStartGoodsContractDt").data("kendoExtMaskedDatePicker").value() == null)){
                //적용일을 모두 입력해 주세요
            ...
            popupWindow = dms.window.popup({
                ...
                ,content :{
                    ...
                    ,data :{
                        "type" :null
                        ,"autoBind" :false
                        , msg :"<spring:message code='sal.btn.releaseReq' var='salBtnReleaseReq' /><spring:message code='global.info.cnfrmMsg' arguments='${salBtnReleaseReq}' />"
                        , btnMsg01 :"<spring:message code='sal.btn.releaseReq' />"
                        , btnMsg02 :"<spring:message code='global.btn.cancel' />"
                        ,"callbackFunc" :function(data){
                            if (data.msg == "01"){
    ...
    $("#btnCancel").kendoButton({    // 용품 - 취소
        click:function(e) {
            $('#itemGrid').data('kendoExtGrid').cancelChanges();
        }
    });
    ...
    // 시작일은 신청 종료일보다 클수가 없다.
    $("#sStartGoodsContractDt").on('change', {from:$("#sStartGoodsContractDt"), to:$("#sEndGoodsContractDt")}, fnChkSearchDate);
    $("#sEndGoodsContractDt").on('change', {from:$("#sStartGoodsContractDt"), to:$("#sEndGoodsContractDt")}, fnChkSearchDate);
    $("#sStartExpcGiEndDt").on('change', {from:$("#sStartExpcGiEndDt"), to:$("#sEndExpcGiEndDt")}, fnChkSearchDate);
    $("#sEndExpcGiEndDt").on('change', {from:$("#sStartExpcGiEndDt"), to:$("#sEndExpcGiEndDt")}, fnChkSearchDate);
    ...
    $("#mainGrid").kendoExtGrid({
       ...
       ,columns:[
          ...
          ,{field:"carlineCd", title:"<spring:message code='global.lbl.carLine' />", attributes:{"class":"ac"}, width:100}  //차종
       ...
       ,dataBound:function(e) {
           ...
           var vGoodsAmt   = 0          // 용품금액
            ...
            $.each(rows, function(idx, row){
                ...
                vSaleAmt += Number(item.saleAmt) * Number(item.goodsCnt);
            });
       ...
       ,columns:[
           ...
           , {field:"goodsCd", title:"<spring:message code='sal.lbl.goodsCd' />", width:100 //용품코드
               ...
               ,editor:function(container, options) {
                   if( (!options.model.isNew() || dms.string.isNotEmpty(options.model.pkgItemCd))
                           &&  Number($("#contractStatCd").val()) >= 20 ){
                       container.context.innerText = options.model.goodsCd;
                       return;
                   }
           ...
           , {
                  field:"goodsCnt", title:"<spring:message code='global.lbl.qty' />", width:80, attributes:{"class":"ar"}
                 ,format:"{0:n0}"
           } //수량
           ...
           ,{field:"dcRate", title:"<spring:message code='global.lbl.dcRate' />", width:90, attributes:{"class":"ar"}, format:"{0:n0}%"} //할인율
           ,{field:"saleAmt", title:"<spring:message code='sal.lbl.saleAmt' />", width:110, attributes:{"class":"ar"}, format:"{0:n2}"} //실제판매가
           ,{
               field:"payTp", title:"<spring:message code='sal.lbl.paysTp' />", width:100, headerAttributes:{"class":"ac"}, attributes:{"class":"ac"}
               ,template:"#= dms.string.strNvl(ioTpObj[payTp]) #"
               ,editor:function (container, options){
            ...
            ,{field:"totAmt", title:"<spring:message code='global.lbl.amt' />", width:110, attributes:{"class":"ar"}, format:"{0:n2}"} //단가
...
function save(mFlag){
    ...
    $.each(param.insertList, function(idx, row){
        ...
        if(row.payTp == ""){
            // 납부방식을 선택하여 주세요.
            dms.notification.info("<spring:message code='sal.lbl.paysTp' var='salLblPaysTp' /><spring:message code='global.info.chkSelectItemMsg' arguments='${salLblPaysTp}' />");
            dataValidate = true;
            return false;
        }
     });
...
sumItemAmt = function(){
    ...
    vCustPayAmt = vGoodsAmt - vPrenSumAmt;
...
bf_gridSumGoodsAmt = function(){
     ...
     rows.each(function(index, row) {
        ...
        if(oItem.saleAmt != null){
           oSaleAmt += Number(oItem.saleAmt) * Number(oItem.goodsCnt);
        }

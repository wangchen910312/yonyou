...
<section class="group">
    ...
    <div class="header_area">
        <h1 class="title_basic">전상운송심사</h1><!-- 전상운송심사 sal.menu.onLineDlrTransfAppr sal.title.onLineDlrTransfAppr-->
        <div class="btn_right">
            ...
            <button class="btn_m btn_delete" id="btnDelete"><spring:message code="global.btn.del" /></button><!-- 폐기 -->
        </div>
    </div>
    <!-- 조회 조건 타이틀 종료 -->
...
/*     $("#sEvalRsltCd").kendoExtDropDownList({
    ...
    confirm = function(appr){
        ...
        rows.each(function(index, row) {
            ...
            vo.push({dlrCd:row.dlrCd, usrId:row.usrId, reqNo:row.reqNo, reqDt:row.reqDt, evalRsltCd:appr});
        });
        ...
        $.ajax({
            url:"<c:url value='/sal/ond/onLineDlrTransfAppr/updateOnLineDlrTransfApprStatus.do' />",
    ...
    save = function(appr){
        ...
        rows.each(function(index, row) {
            ...
            vo.push({dlrCd:row.dlrCd, usrId:row.usrId, reqNo:row.reqNo, reqDt:row.reqDt,
                     evalRsltCd:appr, selfYn:row.selfYn, reptToDlrCd:row.reptToDlrCd, reptToAddr:row.reptToAddr,
                     reptToProvCd:row.reptToProvCd, reptToCityCd:row.reptToCityCd, reptToRecv:row.reptToRecv,
                     reptToIdno:row.reptToIdno, reptToRecvTel:row.reptToRecvTel, carlineCd:row.carlineCd,
                     modelCd:row.modelCd, ocnCd:row.ocnCd, extColorCd:row.extColorCd, intColorCd:row.intColorCd,
                     reqQty:row.reqQty});
        });
        ...
        $.ajax({
            url:"<c:url value='/sal/ond/onLineDlrTransfAppr/saveOnLineDlrTransfAppr.do' />",
            data:JSON.stringify(param),
            type:'POST',
            dataType:'json',
            contentType:'application/json',
    ...
    $("#grid").kendoExtGrid({
        ...
        ,dataSource:{
             transport:{
                read:{
                    url:"<c:url value='/sal/ond/onLineDlrTransfAppr/selectOnLineDlrTransfAppr.do' />"
                }
        ...
        ,columns:[
                 ...
                 ,{field:"dlrCd", title:"<spring:message code='sar.lbl.dlrCd' />", width:80, attributes:{"class":"al"}}//딜러코드
                 ,{field:"dlrNm", title:"<spring:message code='global.lbl.dlrNm' />", width:100, attributes:{"class":"al"}}//딜러명
                 ...
                 ,{field:"modelNm"
                    ...
                    ,editor:function (container, options){
                        ...
                        var responseJson = dms.ajax.getJson({
                            url:"<c:url value='/sal/ond/onLineDlrTransfAppr/selectOndModel.do' />"
                 ...
                 ,{field:"ocnCd"
                     ...
                     ,editor:function (container, options){
                         ...
                         var responseJson = dms.ajax.getJson({
                             url:"<c:url value='/sal/ond/onLineDlrTransfAppr/selectOndOcn.do' />"
                         ...
                         .kendoExtDropDownList({
                             ...
                             ,select:function(e){
                                 ...
                                 var responseJson = dms.ajax.getJson({
                                     url:"<c:url value='/sal/ond/onLineDlrTransfAppr/selectFscpRegGrade.do' />"
                 ...
                 ,{field:"extColorCd"
                     ...
                     ,editor:function (container, options){
                         if(dms.string.strNvl(options.model.ocnCd) == ""){return false;}
                         var responseJson = dms.ajax.getJson({
                             url:"<c:url value='/sal/ond/onLineDlrTransfAppr/selectOndExtColor.do' />"
                 ...
                 ,{field:"intColorCd"
                     ...
                     ,editor:function (container, options){
                         if(dms.string.strNvl(options.model.ocnCd) == ""){return false;}
                         var responseJson = dms.ajax.getJson({
                             url:"<c:url value='/sal/ond/onLineDlrTransfAppr/selectOndIntColor.do' />"
                                 ,data:JSON.stringify({"sCarlineCd":options.model.carlineCd, "sModelCd":options.model.modelCd, "sOcnCd":options.model.ocnCd})
                                 ,async:false
                         });

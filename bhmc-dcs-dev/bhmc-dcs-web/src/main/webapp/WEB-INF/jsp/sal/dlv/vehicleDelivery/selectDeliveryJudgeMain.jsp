...
<section class="group">
    <div id="tabstrip" class="tab_area tab_iframe">
        <ul>
            <li id="1st_tab" class="k-state-active"><spring:message code="sal.title.exceptionDeliveyEval" /></li>   <!-- 이상출고심사 -->
            <li id="2nd_tab"><spring:message code="sal.title.tacticsDeliveryApprove" /></li>                        <!-- 전략출고심사 -->
        </ul>
        <div class="mt0">
...
selectTab = function(tabId){
    ...
    if (tabId == "1st_tab") {
        if($("#iframeStatus01").attr("src") == ""){
            iframeUrl = "<c:url value='/sal/fas/coercionApproval/selectCoercionApprovalMain.do' />";
            $("#iframeStatus01").attr("height", windowHeight - 80);
            $("#iframeStatus01").attr("src", iframeUrl);
        }
    }else{
        if($("#iframeStatus02").attr("src") == ""){
            iframeUrl = "<c:url value='/sal/dlv/vehicleDelivery/selectTacticsDeliveryApproveMain.do' />";

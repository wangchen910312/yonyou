...
var options = parent.qualityClaimPopupWindow.options.content.data,
    ...
    carModelList.push({cmmCd:"", cmmCdNm:" "});
    <c:forEach var="obj" items="${carModelList}" varStatus="status">
    carModelList.push({cmmCd:"${obj.cmmCd}", cmmCdNm:"${obj.cmmCdNm}"});
    </c:forEach>
    
    claimTypeList.push({cmmCd:"", cmmCdNm:" "});
    <c:forEach var="obj" items="${claimTypeList}" varStatus="status">
    claimTypeList.push({cmmCd:"${obj.cmmCd}", cmmCdNm:"${obj.cmmCdNm}"});
    </c:forEach>
    
    dealerClaimWayList.push({cmmCd:"", cmmCdNm:" "});
    <c:forEach var="obj" items="${dealerClaimWayList}" varStatus="status">
    dealerClaimWayList.push({cmmCd:"${obj.cmmCd}", cmmCdNm:"${obj.cmmCdNm}"});
    </c:forEach>
    
    prcCodeList.push({cmmCd:"", cmmCdNm:" "});
    <c:forEach var="obj" items="${prcCodeList}" varStatus="status">
    prcCodeList.push({cmmCd:"${obj.cmmCd}", cmmCdNm:"${obj.cmmCdNm}"});
    </c:forEach>
    
    deliveryTypeList.push({cmmCd:"", cmmCdNm:" "});
    <c:forEach var="obj" items="${deliveryTypeList}" varStatus="status">
    deliveryTypeList.push({cmmCd:"${obj.cmmCd}", cmmCdNm:"${obj.cmmCdNm}"});
    </c:forEach>
        ...
        $("#btnSend").kendoButton({
            click:function(e){
              ...
              //if (!partsJs.validate.getAllPluginObjInfo($("#genInfoForm")[0])) {
                ...
                $.ajax({
                        url:"<c:url value='/par/pcm/claim/sendQualityClaim.do' />"
                      , type:"POST"
                      , data:saveData
                      , dataType:"json"
                      , contentType:'application/json'
     ...
     ***************************************************************************/
     ...
     function checkValidation(){
         ...
         if(dms.string.isEmpty($("#arrivalDate").data("kendoExtMaskedDatePicker").value())){
           //도착일자는 필수 입력 사항입니다.
             dms.notification.warning("<spring:message code='par.lbl.arrvDt' var='purcOrdTp' /><spring:message code='global.info.required.field' arguments='${arrvDt}'/>");
             return false;
         }

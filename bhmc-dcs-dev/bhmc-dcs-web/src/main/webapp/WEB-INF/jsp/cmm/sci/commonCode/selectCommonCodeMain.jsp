...
<section class="group">
    <div class="header_area">
        ...
        <div class="btn_right">
            <dms:access viewId="VIEW-I-12163" hasPermission="${dms:getPermissionMask('READ')}">
    ...
    <div class="header_area">
        ...
        <div class="btn_right">
            <dms:access viewId="VIEW-I-12162" hasPermission="${dms:getPermissionMask('READ')}">
                <button type="button" class="btn_s" id="btnSaveGroup"><spring:message code='global.btn.save' /><!-- 저장 --></button>
            </dms:access>
            <dms:access viewId="VIEW-I-12161" hasPermission="${dms:getPermissionMask('READ')}">
                <button type="button" class="btn_s" id="btnAddGroup"><spring:message code='global.btn.rowAdd' /><!-- 행추가 --></button>
            </dms:access>
            <dms:access viewId="VIEW-I-12160" hasPermission="${dms:getPermissionMask('READ')}">
                <button type="button" class="btn_s" id="btnDelGroup"><spring:message code='global.btn.rowDel' /><!-- 행삭제 --></button>
            </dms:access>
            <dms:access viewId="VIEW-I-12159" hasPermission="${dms:getPermissionMask('READ')}">
    ...
    <div class="header_area">
        ...
        <div class="btn_right">
            <dms:access viewId="VIEW-I-12158" hasPermission="${dms:getPermissionMask('READ')}">
                <button type="button" class="btn_s" id="btnSaveCode"><spring:message code='global.btn.save' /><!-- 저장 --></button>
            </dms:access>
            <dms:access viewId="VIEW-I-12157" hasPermission="${dms:getPermissionMask('READ')}">
                <button type="button" class="btn_s" id="btnAddCode"><spring:message code='global.btn.rowAdd' /><!-- 행추가 --></button>
            </dms:access>
            <dms:access viewId="VIEW-I-12156" hasPermission="${dms:getPermissionMask('READ')}">
                <button type="button" class="btn_s" id="btnDelCode"><spring:message code='global.btn.rowDel' /><!-- 행삭제 --></button>
            </dms:access>
            <dms:access viewId="VIEW-I-12155" hasPermission="${dms:getPermissionMask('READ')}">
...
//공통코드그룹 언어설정 설정 팝업
var cmmGrpTextPopupWin;
function cmmGrpTextPopup(that) {
    cmmGrpTextPopupWin = dms.window.popup({
        windowId:"cmmGrpTextPopupWin"
        ,title:"<spring:message code='cmm.title.cmmGrpLang' />"
        ,content:{
            url:"<c:url value='/cmm/sci/commonCode/selectCommonCodeGroupTxtPopup.do' />"
            , data:{
                "autoBind":true
                ,"cmmGrpCd":that
                ,"selectable":"multiple"
...
$(document).ready(function() {
    ...
    $("#btnSaveGroup").kendoButton({
        click:function(e) {
            ...
            if (grid.gridValidation()) {
                $.ajax({
                    url:"<c:url value='/cmm/sci/commonCode/multiCommonCodeGroups.do' />"
    ...
    $("#groupGrid").kendoExtGrid({
        dataSource:{
            ...
            ,change:function(e){
                var grid = $("#groupGrid").data("kendoExtGrid");
                if(e.action === "itemchange" && e.field === "sysCdYn"){
                    var sysCdYn = e.items[0].sysCdYn;
                    var cmmGrpCd = e.items[0].cmmGrpCd;
                    $.each(tempComGroupList, function(idx, obj){
                        if(cmmGrpCd == obj.cmmGrpCd){
                            if(sysCdYn == "Y" && obj.sysCdYn == "N"){
                                e.items[0].set("sysCdYn", "N");
                                dms.notification.info("<spring:message code='global.err.chkSysCdYn'/>");
                            }
                        }
                    });
                }
        ...
        ,editableOnlyNewExcludeColumns:["cmmGrpCdNm","sysCdYn","useYn","remark","remarkTitleNm"]
        ,columns:[
            {title:"", width:80, sortable:false
                ,attributes:{"class":"ac"}
                ,template:"<span class='txt_label bg_yellow' onClick='cmmGrpTextPopup(\"#=cmmGrpCd#\")'><spring:message code='global.lbl.set.lang' /></span>"
            }
            ...
            ,{field:"remark", title:"<spring:message code='global.lbl.remark' />", width:300, sortable:false }
            ,{field:"remarkTitleNm", title:"<spring:message code='cmm.lbl.remarkTitleNm' />", sortable:false }
        ]
        ,dataBound:function(e){
            tempComGroupList = [];
            $.each(e.sender.dataSource.data(), function(idx, obj){
                var cmmGrpCd = obj.cmmGrpCd;
                var sysCdYn = obj.sysCdYn;
                tempComGroupList.push({"cmmGrpCd":cmmGrpCd, "sysCdYn":sysCdYn});
            });
        }
        ,change:function(e){

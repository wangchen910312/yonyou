        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <th scope="row"><spring:message code='crm.lbl.RepairOrderDate' /></th> <!-- 정비일자 -->
                    <td>
                        <div class="form_float">
                            <div class="date_left">
                                <input id="sServiceFromDt" name="sServiceFromDt" class="form_datepicker ac">
                                <span class="txt_from">~</span>
                            </div>
                            <div class="date_right">
                                <input id="sServiceToDt" name="sServiceToDt" class="form_datepicker ac">
                            </div>
                        </div>
                    </td>
                    <th scope="row"><spring:message code='crm.lbl.joinApproveDt' /></th> <!-- 회원가입일자 -->
                    <td>
                        <div class="form_float">
                            <div class="date_left">
                                <input id="sJoinFromDt" name="sJoinFromDt" class="form_datepicker ac">
                                <span class="txt_from">~</span>
                            </div>
                            <div class="date_right">
                                <input id="sJoinToDt" name="sJoinToDt" class="form_datepicker ac">
                            </div>
                        </div>
                    </td>
...
$(document).ready(function() {
    ...
    $("#btnSearch").kendoButton({
        click:function(e){
            ...
            var sServiceFromDt = $("#sServiceFromDt").val();
            var sServiceToDt = $("#sServiceToDt").val();
            var sJoinFromDt = $("#sJoinFromDt").val();
            var sJoinToDt = $("#sJoinToDt").val();
            var sCarlineCd = $("#sCarlineCd").val();

            if(dms.string.isEmpty(sServiceFromDt) || dms.string.isEmpty(sServiceToDt)){
    ...
    //정비일자
    $("#sServiceFromDt").kendoExtMaskedDatePicker({
        format:"<dms:configValue code='dateFormat' />"
        ,dateValidMessage:"<spring:message code='ser.lbl.date' var='dateMsg' /><spring:message code='global.err.invalid' arguments='${dateMsg}' />"
        ,value:"${fromDt}"
    });

    $("#sServiceToDt").kendoExtMaskedDatePicker({
        format:"<dms:configValue code='dateFormat' />"
        ,dateValidMessage:"<spring:message code='ser.lbl.date' var='dateMsg' /><spring:message code='global.err.invalid' arguments='${dateMsg}' />"
        ,value:"${toDt}"
    });
    ...
    //회원가입일자
    $("#sJoinFromDt").kendoExtMaskedDatePicker({
        format:"<dms:configValue code='dateFormat' />"
        ,dateValidMessage:"<spring:message code='ser.lbl.date' var='dateMsg' /><spring:message code='global.err.invalid' arguments='${dateMsg}' />"
        ,value:"${fromDt}"
    });

    $("#sJoinToDt").kendoExtMaskedDatePicker({
        format:"<dms:configValue code='dateFormat' />"
        ,dateValidMessage:"<spring:message code='ser.lbl.date' var='dateMsg' /><spring:message code='global.err.invalid' arguments='${dateMsg}' />"
        ,value:"${toDt}"
    });

    $("#sServiceFromDt").kendoMaskedTextBox({
        mask:"####-##-##"
    });
    $("#sServiceToDt").kendoMaskedTextBox({
        mask:"####-##-##"
    });

    $("#sJoinFromDt").kendoMaskedTextBox({
        mask:"####-##-##"
    });
    $("#sJoinToDt").kendoMaskedTextBox({
        mask:"####-##-##"
    });
...
function subTotalStatusCall(){
    ...
    var subTotalStatusUrl = "<c:url value='/ReportServer?reportlet=crm/report/selectServicePointUseRateReport.cpt' />&op=view";
    var param = "";

    param += "&sServiceFromDt="+sServiceFromDt;
    param += "&sServiceToDt="+sServiceToDt;
    param += "&sJoinFromDt="+sJoinFromDt;
    param += "&sJoinToDt="+sJoinToDt;

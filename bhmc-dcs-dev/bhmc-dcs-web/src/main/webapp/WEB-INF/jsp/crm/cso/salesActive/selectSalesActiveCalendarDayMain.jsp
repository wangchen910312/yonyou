...
<%@taglib prefix="dms" uri="/WEB-INF/tlds/taglibs-dms"%>
            ...
            <section class="group">
                <div class="header_area">
                    <h1 class="title_basic"><spring:message code="crm.menu.salesActiveScheduler" /><!-- 영업활동 스케줄 --></h1>
                </div>
...
<!-- 영업활동 등록 팝업 ( data set 하기 위해서 처리 )-->
<script id="salesActiveInfoPopup" type="text/x-kendo-template">
<div>
    <input type="hidden" id="schedulerSeq" name="schedulerSeq" />
    <input type="hidden" id="schedulerTp" name="schedulerTp" />
...
<!-- //영업스케줄 팝업 -->
            ...
            <script type="text/javascript">
                ...
                $(document).ready(function() {
                    ...
                    // 영업스케줄 등록 팝업
                    function salesActivePopup() {

                        var schedulerSeq = $("#schedulerSeq").val();
                        var schedulerTp = $("#schedulerTp").val();
                        var start = new Date($("#start").val());

                        // 스케쥴러 데이터 set 한다음에 스케줄러에서 제공되는 form 닫기
                        $("#scheduler").data("kendoScheduler").cancelEvent();
                        console.log("schedulerSeq1:"+schedulerSeq);

                        // 신규 등록일때 validation 시작
                        if ( dms.string.isEmpty(schedulerSeq) ) {
                        ...
                        console.log("schedulerTp:"+schedulerTp);
                    ...
                    $("#scheduler").kendoScheduler({
                        ...
                        ,views:[
                            "day"
                            //,"week"
                            //,{ type:"month", selected:true, eventHeight:20 }
                        ]
                        ,showWorkHours:false
                        ...
                        ,resources:[
                                    {
                                         field:"schedulerTp"       // 01(내방) / 02(시승) / 03(활동)
                                        ,dataSource:[
                                             { text:"", value:"01", color:"#f8a398" }    //내방
                                            ,{ text:"", value:"02", color:"#56ca85" }    //시승
                                            ,{ text:"", value:"03", color:"#aabbcc" }    //활동
                                        ]
                                    }
                                    ,{
                                         field:"scId"
                        ...
                        ,editable:{
                            // 더블클릭시 기존 레이어 팝업 말고 다른팝업 띄울려면 template 로 설정.
                            template:$("#salesActiveInfoPopup").html()
                            ,move:false
                            ,window:{
                                title:"<spring:message code='crm.menu.salesActiveAdd' />"  // 영업활동 등록
                                ,animation:false
                                ,open:salesActivePopup
                            }
                        }
                    });

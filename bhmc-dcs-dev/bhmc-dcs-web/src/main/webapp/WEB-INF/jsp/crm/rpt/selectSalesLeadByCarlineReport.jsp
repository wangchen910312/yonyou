        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <th scope="row"><spring:message code='sal.lbl.purcRegDt' /></th> <!-- 거래일자 -->
                    <td>
                    ...
                    <th scope="row"><spring:message code='crm.lbl.opptRegistDt' /></th> <!-- 리드배포일자 -->
                    <td>
                        <div class="form_float">
                            <div class="date_left">
                                <input id="sLeadFromDt" name="sLeadFromDt" class="form_datepicker ac">
                                <span class="txt_from">~</span>
                            </div>
                            <div class="date_right">
                                <input id="sLeadToDt" name="sLeadToDt" class="form_datepicker ac">
                            </div>
                        </div>
                    </td>
                ...
                <tr>
                    <th scope="row"><spring:message code='crm.lbl.bhmcLeadyn' /></th> <!-- 제조사 리드여부    -->
                        <td>
                            <input id="sBhmcYn" type="text" class="form_comboBox">
                        </td>
                    <th></th>
                    <td></td>
                    <th></th>
...
$(document).ready(function() {
    ...
    $("#btnSearch").kendoButton({
        click:function(e){
            ...
            var sLeadFromDt = $("#sLeadFromDt").val();
            var sLeadToDt = $("#sLeadToDt").val();
            var sCarlineCd = $("#sCarlineCd").val();
            var sBhmcYn = $("#sBhmcYn").val();

            if(dms.string.isEmpty(sLeadFromDt) || dms.string.isEmpty(sLeadToDt)){
    ...
    //리드배포일자
    $("#sLeadFromDt").kendoExtMaskedDatePicker({
        format:"<dms:configValue code='dateFormat' />"
        ,dateValidMessage:"<spring:message code='ser.lbl.date' var='dateMsg' /><spring:message code='global.err.invalid' arguments='${dateMsg}' />"
        ,value:"${fromDt}"
    });

    $("#sLeadToDt").kendoExtMaskedDatePicker({
        format:"<dms:configValue code='dateFormat' />"
        ,dateValidMessage:"<spring:message code='ser.lbl.date' var='dateMsg' /><spring:message code='global.err.invalid' arguments='${dateMsg}' />"
        ,value:"${toDt}"
    });
    ...
    $("#sLeadFromDt").kendoMaskedTextBox({
        mask:"####-##-##"
    });
    $("#sLeadToDt").kendoMaskedTextBox({
        mask:"####-##-##"
    });
...
function subTotalStatusCall(){
    ...
    var subTotalStatusUrl = "<c:url value='/ReportServer?reportlet=crm/report/selectSalesLeadByCarlineReport.cpt' />&op=view";
    var param = "";

    param += "&sSearchFromDt="+sSearchFromDt;
    param += "&sSearchToDt="+sSearchToDt;
    param += "&sLeadFromDt="+sLeadFromDt;
    param += "&sLeadToDt="+sLeadToDt;
    ...
    if(dms.string.isNotEmpty(sBhmcYn)){
        param += "&sBhmcYn="+sBhmcYn;
    }
    console.log(param);

...
public class MarketingCampaignController extends HController {
    ...
    public String selectMarketingCampaignMain(Model model
        ...
        model.addAttribute("budGetList",   commonCodeService.selectCommonCodesByCmmGrpCd("CRM707", null, langCd));  //예산코드
     ...
     * @return
     */
    @RequestMapping(value = "/crm/mcm/marketingCampaign/selectMarketingCampaignMain2.do", method = RequestMethod.GET)
    public String selectMarketingCampaignMain2(Model model
            , @RequestParam(value="makCd", defaultValue="") String makCd
    ) throws Exception {
        ...
        return "/crm/mcm/marketingCampaign/selectMarketingCampaignMain2";
    }
    ...
    @ResponseBody
    public boolean multiActives(@Validated @RequestBody MarketingCampaignSaveExVO saveVO) throws Exception {
    ...
    public int multiTargets(@Validated @RequestBody MarketingCampaignVO activeVO)throws Exception {
        ...
        activeVO.setDlrCd(activeVO.gettDlrCd());
        activeVO.setMakCd(activeVO.gettMakCd());
    ...
    public String insertSignEx(@Validated @RequestBody MarketingCampaignSignVO marketingCampaignSignVO, Model model) throws Exception {
        ...
        if(marketingCampaignService.selectWorkItemsByConditionCnt(searchVO) <= 0){
            ...
            //결재할 사람에게 알람을 보낸다.
//            marketingCampaignService.pushSignAlarms(signVO);
    ...
    public @ResponseBody Map<String, Object> selectSurveyPopups(@RequestBody CampaignResultSearchVO searchVO) throws Exception {
        ...
        searchVO.setsIvstTp(searchVO.getsIvstTp());
...
//    /**
//     * 캠페인에 따른 추출대상자 데이터를 추출차수별로 조회한다.
//     * @param searchVO - 조회 조건을 포함하는 MarketingCampaignSearchVO
//     * @return
//     */
//    @RequestMapping(value = "/crm/cmm/crmTargExtract/selectCrmTargExtractDetailPopups.do", method = RequestMethod.POST)
//    @ResponseBody
//    public SearchResult selectCrmTargExtractDetailPopups(@RequestBody MarketingCampaignSearchVO searchVO) throws Exception {
//
//        SearchResult result = new SearchResult();
//
//        result.setTotal(marketingCampaignService.selectCrmTargExtractPopupsByConditionCnt(searchVO));
...
//    }
    ...
    public SearchResult insertVinUploadFile(
        ...
        if(uploadFile.getSize() > 0){
            ...
            try{
                ...
                if(progressId.equals("VIN")){ // VIN 정보 업로드시
                    MarketingCampaignVO marketingCampaignVO = new MarketingCampaignVO();
                    marketingCampaignVO.setDlrCd(LoginUtil.getDlrCd());
                    marketingCampaignVO.setMakCd(makCd);
                    marketingCampaignService.deleteVinTmpExcelData(marketingCampaignVO);
                }

                for(MarketingCampaignVinExcelVO vo : list){
                    boolean addYn = false;

                    if(progressId.equals("VIN")){
                        addYn = (StringUtil.nullConvert(vo.getVinNo()).equals("")) ? false : true;

                        if(addYn){ //VIN 정보 TEMP Table 저장
                            MarketingCampaignVO marketingCampaignVO = new MarketingCampaignVO();
                        ...
                        addYn = (StringUtil.nullConvert(vo.getLtsModelCd()).equals("")) ? false : true;
                    }

                    if(addYn){
                        //VIN 정보 TEMP Table 저장
                        list2.add(vo);
                    }
                }
                ...
                if(progressId.equals("VIN")){
                    MarketingCampaignSearchVO searchVO = new MarketingCampaignSearchVO();
                    searchVO.setsCrNo(makCd);
                    searchVO.setsDlrCd(LoginUtil.getDlrCd());
                    result.setTotal(marketingCampaignService.selectVinTmpByConditionCnt(searchVO));
                    if(result.getTotal() != 0){
                        result.setData(marketingCampaignService.selectVinTmpByCondition(searchVO));
                    }
                }else{
     ...
     * @return
     */
    @RequestMapping(value = "/crm/mcm/marketingCampaign/selectDealers.do", method = RequestMethod.POST)
    @ResponseBody
    public SearchResult selectDealers(@RequestBody MarketingCampaignSearchVO searchVO) throws Exception {

        SearchResult result = new SearchResult();
        result.setTotal(marketingCampaignService.selectDealersByConditionCnt(searchVO));

        if(result.getTotal() != 0){
            List<MarketingCampaignVO> selectDealersByCondition = marketingCampaignService.selectDealersByCondition(searchVO);

            result.setData(selectDealersByCondition);
        }
...
//
//        SearchResult result = new SearchResult();
//        searchVO.setsDmsUseYn("Y");
//        result.setTotal(dealerService.selectDealersByConditionCnt(searchVO));
//
//        if(result.getTotal() != 0){
//            List<DealerVO> selectDealersByCondition = dealerService.selectDealersByCondition(searchVO);
//
//            result.setData(selectDealersByCondition);
//        }
//
...
//    }
     ...
     * @return
     */
    @RequestMapping(value = "/crm/mcm/marketingCampaign/selectDlrs.do", method = RequestMethod.POST)
    @ResponseBody
    public SearchResult selectDlrs(@RequestBody MarketingCampaignSearchVO searchVO) throws Exception {

        SearchResult result = new SearchResult();

        result.setTotal(marketingCampaignService.selectDlrsByConditionCnt(searchVO));

        if(result.getTotal() != 0){

            List<MarketingCampaignVO> selectDlrsByCondition = marketingCampaignService.selectDlrsByCondition(searchVO);

            result.setData(selectDlrsByCondition);
        }
     ...
     * @return 등록 대상자 수
     */
    @RequestMapping(value = "/crm/mcm/marketingCampaign/saveDlrCdData.do", method = RequestMethod.POST)
    @ResponseBody
    public int saveDlrCdData(@Validated @RequestBody MarketingCampaignSaveExVO activeVO)throws Exception {

        if (StringUtils.isEmpty(activeVO.gettDlrCd())){activeVO.settDlrCd(LoginUtil.getDlrCd());}

        return marketingCampaignService.saveDlrCdData(activeVO);
    ...
    /**
     * 마케팅 캠페인 캘린더 화면을 출력한다.
     * @return
     */
    @RequestMapping(value = "/crm/mcm/marketingCampaign/selectMarketingCampaignCalendarMain.do", method = RequestMethod.GET)
    public String selectMarketingCampaignCalendarMain(Model model) throws Exception {

        String sysDate = DateUtil.getDate("yyyy-MM-dd");
        model.addAttribute("sysDate", sysDate);
        model.addAttribute("makTpList", commonCodeService.selectCommonCodesByCmmGrpCd("CRM702", null, LocaleContextHolder.getLocale().getLanguage()));  //유형

        return "/crm/mcm/marketingCampaign/selectMarketingCampaignCalendarMain";
    }
     ...
     * @return
     */
    @RequestMapping(value = "/crm/mcm/marketingCampaign/selectMarketingCampaignCalendars.do", method = RequestMethod.POST)
    @ResponseBody
    public SearchResult selectMarketingCampaignCalendars(@RequestBody MarketingCampaignSearchVO searchVO) throws Exception {
        ...
        if(result.getTotal() != 0){
            result.setData(marketingCampaignService.selectMarketingCampaignsByCondition(searchVO));
        }
    ...
    public String selectUsersForTaskName(@RequestBody UserSearchVO searchVO) throws Exception {
        ...
        if (StringUtils.isEmpty(userVO.getTskCd())) {
            ...
            TaskVO taskVO = taskService.selectTaskByKey("10", userVO.getTskCd());   // cmpTp 회사구분 '10':DCS(본사), '20':DMS(딜러)

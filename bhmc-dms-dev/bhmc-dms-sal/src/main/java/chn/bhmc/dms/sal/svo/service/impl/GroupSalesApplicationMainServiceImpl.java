...
import chn.bhmc.dms.sal.svo.service.GroupSalesApplicationMainService;
...
@Service("groupSalesApplicationMainService")
public class GroupSalesApplicationMainServiceImpl extends HService implements GroupSalesApplicationMainService , JxlsSupport{
...
//        params.put(sConfirmDtE, sConfirmDtE);
        ...
        searchVO.setsLangCd(LocaleContextHolder.getLocale().getLanguage());
        List<GroupSalesApplicationVO> list = groupSalesApplicationDAO.selectGroupSalesApplicationMainByCondition(searchVO);

        HashMap<String, String> approveNmMap = new HashMap<String, String>();
        for(CommonCodeVO vo : commonCodeService.selectCommonCodesByCmmGrpCd("SAL184", null, LocaleContextHolder.getLocale().getLanguage())){
            approveNmMap.put(vo.getCmmCd(), vo.getCmmCdNm());
        }

        for(GroupSalesApplicationVO vo : list){
            vo.setApproveCd(approveNmMap.get(vo.getApproveCd()));
        }

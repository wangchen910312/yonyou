...
public class AskHelpCarController extends HController {
    ...
    AskHelpCarService askHelpCarService;
    ...
    public String selectAskHelpCarMain(Model model) throws Exception{
        ...
        UserVO userVO = userService.selectUserByKey(LoginUtil.getUserId());
        model.addAttribute("userNm", userVO.getUsrNm());
        model.addAttribute("userTelNo", StringUtils.isBlank(userVO.getTelNo1())?userVO.getHpNo():userVO.getTelNo1());
    ...
    @ResponseBody
    public SearchResult selectAskHelpCarPopupEvls(@RequestBody BtoCarInfoSearchVO searchVO) throws Exception {
        searchVO.setDlrCd(LoginUtil.getDlrCd());      // 딜러코드
        searchVO.setUseYn("Y");                       // 사용여부

        SearchResult result = new SearchResult();
        List<BtoCarInfoVO>  list = btoCreateOrderService.selectBlockNewCarLooksModelListByCondition(searchVO);

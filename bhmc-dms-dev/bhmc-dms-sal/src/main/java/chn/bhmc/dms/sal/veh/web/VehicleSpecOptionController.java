...
public class VehicleSpecOptionController extends HController {
    ...
    /**
     * VehicleSpec Option 목록 데이터를 조회한다.
     * @param searchVO - 조회 조건을 포함하는 VehicleSearchVO
     * @return
     */
    @RequestMapping(value = "/selectVehicleSpecOption.do", method = RequestMethod.POST)
    @ResponseBody
    public SearchResult selectVehicleSpecOption(@RequestBody VehicleSearchVO searchVO) throws Exception {

        SearchResult result = new SearchResult();

        result.setTotal(vehicleSpecOptionService.selectVehicleOptionByConditionCnt(searchVO));

        if(result.getTotal() != 0){
            result.setData(vehicleSpecOptionService.selectVehicleOptionByCondition(searchVO));
        }

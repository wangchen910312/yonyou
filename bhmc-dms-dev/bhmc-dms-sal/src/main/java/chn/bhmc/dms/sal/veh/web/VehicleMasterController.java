...
public class VehicleMasterController extends HController {
     ...
     * 엔진히스토리리스트를 가져온다.
     */
    @RequestMapping(value = "/selectVehicleMasterEngineHistory.do", method = RequestMethod.POST)
    @ResponseBody
    public SearchResult selectVehicleMasterEngineHistory(@RequestBody VehicleMasterSearchVO searchVO) throws Exception {
        SearchResult result = new SearchResult();
        result.setData(vehicleMasterService.selectVehicleMasterEngineHistory(searchVO));
    ...
    /**
     * <p>Title:绑定车架号-查询车辆信息 </p >
     * @author TangWei 2021年4月15日 下午4:34:15
     * @param 
     * @return
     */
    @RequestMapping(value = "/sal/veh/selectBindingVinNoCarInfo.do", method = RequestMethod.POST)
    @ResponseBody
    public SearchResult selectBindingVinNoCarInfo(@RequestBody VehicleMasterSearchVO searchVO) throws Exception {
        SearchResult result = new SearchResult();
        searchVO.setsDlrCd(LoginUtil.getDlrCd());
        result.setTotal(vehicleMasterService.selectBindingVinNoCarInfoCnt(searchVO));
        if(result.getTotal() != 0){
            result.setData(vehicleMasterService.selectBindingVinNoCarInfoData(searchVO));
        }
        return result;
    }
}
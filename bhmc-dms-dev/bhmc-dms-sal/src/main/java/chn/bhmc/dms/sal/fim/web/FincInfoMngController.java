...
public class FincInfoMngController extends HController {
    ...
    @ResponseBody
    public SearchResult selectModel(@RequestBody VehicleSpecSearchVO searchVO) throws Exception {
        searchVO.setUseYn("Y");
    ...
    @ResponseBody
    public SearchResult selectFincInfoMngCRMSearch(@RequestBody FincInfoMngSearchVO searchVO) throws Exception {
        SearchResult result = new SearchResult();

        result.setTotal(fincInfoMngService.selectCustFincInfosByConditionCnt(searchVO));

        if(result.getTotal() != 0){
            result.setData(fincInfoMngService.selectCustFincInfosByCondition(searchVO));
        }

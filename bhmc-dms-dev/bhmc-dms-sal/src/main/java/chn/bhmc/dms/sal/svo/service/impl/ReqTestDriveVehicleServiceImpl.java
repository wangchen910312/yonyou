...
public class ReqTestDriveVehicleServiceImpl extends HService implements ReqTestDriveVehicleService {
    ...
    public int multiReqTestDriveVehiclePopups(ReqTestDriveVehicleSaveVO saveVO) throws Exception{
        ...
        //if(StringUtil.nullConvert(contVO.getApproveCd()).equals("B")){

        ReqTestDriveVehiclePopupCarVO carVO = new ReqTestDriveVehiclePopupCarVO();
        carVO.setDlrCd(contVO.getDlrCd());
        carVO.setUsrId(contVO.getUsrId());
        carVO.setReqNo(contVO.getReqNo());

        //프로시저 추가[구원차/대부차주문신청정보수신1]
        reqTestDriveVehicleDAO.procDemoCarReq1(carVO);

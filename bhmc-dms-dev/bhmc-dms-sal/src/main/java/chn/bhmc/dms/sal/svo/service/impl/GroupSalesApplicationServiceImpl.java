...
import chn.bhmc.dms.sal.veh.vo.ApptionVehicleSearVO;
...
public class GroupSalesApplicationServiceImpl extends HService implements GroupSalesApplicationService , JxlsSupport{
    ...
    public GroupSalesApplicationVO multiGroupSalesApplication(GroupSalesApplicationSaveVO saveVO) throws Exception{
        ...
        String usrId = grpSaleAppVO.getUsrId();
        //R19082700486 销售申请重复vin_no 数据校验  贾明  2019-9-4 start
        //String  approveCd = grpSaleAppVO.getApproveCd();
        //当批准时不做校验
        if(!grpSaleAppVO.getApproveCd().equals("C")){
        	String  applicaNum  = grpSaleAppVO.getApplicaNum();
            for( GroupSalesApplicationVehVO vehVO : vehicleListVO ){
            	 Map<String, Object> message = new HashMap<String, Object>();
                 message.put("I_VIN_NO1", vehVO.getVinNo().substring(0, 11));
                 message.put("I_VIN_NO2", vehVO.getVinNo().substring(11,vehVO.getVinNo().length()));
                 Data receiveData = salesCommonCamelService.executeCamelClentSearchResult(message,"SAL237");
                 
                 List<ApptionVehicleSearVO> apptionVehicleSearVO = new ArrayList<ApptionVehicleSearVO>();
                 if (receiveData.getFooter().getIfResult().equals("Z")) {
                	 apptionVehicleSearVO = receiveData.readMessages(ApptionVehicleSearVO.class);
                   for(ApptionVehicleSearVO listVO : apptionVehicleSearVO){
                	   if(!listVO.getReqSalNo().equals("")){
                		   if(null==applicaNum || ""==applicaNum){
                			   throw processException("global.info.repeatSubmRemi"
                                       , new String[]{vehVO.getVinNo()
                                                     ,listVO.getDlrCd()
                                                     ,listVO.getReqSalNo()});
                		   }
                		   
                		   if(null!=applicaNum || ""!=applicaNum){
                			  if(!listVO.getApproveCd().equals("D")){
                				  throw processException("global.info.repeatSubmRemi"
                                          , new String[]{vehVO.getVinNo()
                                                        ,listVO.getDlrCd()
                                                        ,listVO.getReqSalNo()});
      ...
      //R19082700486 销售申请重复vin_no 数据校验  贾明  2019-9-4 end
        if(StringUtils.isBlank(usrId)){
            usrId = LoginUtil.getUserId();
            grpSaleAppVO.setUsrId(usrId);
        }
        ...
        if(grpSaleAppVO.getApproveCd().equals("B") || grpSaleAppVO.getApproveCd().equals("C")){
            ...
            if(grpSaleAppVO.getApproveCd().equals("B")){
                groupSalesApplicationDAO.updateGroupSalesApplicationCnfrm(grpSaleAppVO);
                groupSalesApplicationDAO.deleteGroupSalesApplicationDetJdg(grpSaleAppVO);  // Delete SA_0412T
            }
    ...
    */
    ...
    public void initJxlsContext(Context context, HMap params) throws Exception {
        ...
        Date sApplyDtS = null;
        Date sApplyDtE = null;
        Date sConfirmDtS = null;
        Date sConfirmDtE = null;

        if(params.get("sApplyDtS") != null){
            sApplyDtS = DateUtil.convertToDate((String)params.get("sApplyDtS"), "yyyyMMdd");
        }
        if(params.get("sApplyDtE") != null){
            sApplyDtE = DateUtil.convertToDate((String)params.get("sApplyDtE"), "yyyyMMdd");
        }
        if(params.get("sConfirmDtS") != null){
            sConfirmDtS = DateUtil.convertToDate((String)params.get("sConfirmDtS"), "yyyyMMdd");
        }
        if(params.get("sConfirmDtE") != null){
            sConfirmDtE = DateUtil.convertToDate((String)params.get("sConfirmDtE"), "yyyyMMdd");
        }

//        params.put(sApplyDtS, sApplyDtS);
//        params.put(sApplyDtE, sApplyDtE);
//        params.put(sConfirmDtS, sConfirmDtS);
//        params.put(sConfirmDtE, sConfirmDtE);

        GroupSalesApplicationSearchVO searchVO = new GroupSalesApplicationSearchVO();

        ObjectUtil.convertMapToObject(params, searchVO, "beanName", "templateFile", "fileName");

        searchVO.setsApplyDtS(sApplyDtS);
        searchVO.setsApplyDtE(sApplyDtE);
        searchVO.setsConfirmDtS(sConfirmDtS);
        searchVO.setsConfirmDtE(sConfirmDtE);

        searchVO.setsLangCd(LocaleContextHolder.getLocale().getLanguage());

        List<GroupSalesApplicationVO> list = groupSalesApplicationDAO.selectGroupSalesApplicationByCondition(searchVO);

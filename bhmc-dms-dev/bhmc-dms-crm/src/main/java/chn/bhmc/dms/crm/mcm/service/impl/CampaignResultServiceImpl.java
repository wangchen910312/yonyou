...
public class CampaignResultServiceImpl extends HService implements CampaignResultService, JxlsSupport {
    ...
    public int selectCampaignResultMgmtsByConditionCnt(CampaignResultSearchVO searchVO) throws Exception {
        ...
        return campaignResultDAO.selectCampaignResultMgmtsByConditionCnt(searchVO);
    }
    ...
    public List<CampaignResultVO> selectCampaignResultMgmtsByCondition(CampaignResultSearchVO searchVO) throws Exception {
        ...
        return campaignResultDAO.selectCampaignResultMgmtsByCondition(searchVO);
    }
     ...
     */
    @Override
    public void multiCampaign01AnswerSurveys(CampaignResultSaveVO obj) throws Exception {
        String dlrCd = LoginUtil.getDlrCd();
        String pltCd = LoginUtil.getPltCd();
        String userId = LoginUtil.getUserId();
        String makCd = obj.getMakCd();
        String stsfIvstYn = obj.getSatisFactionMobileResultVO().getStsfIvstYn();
        String cmplReasonCont = obj.getSatisFactionMobileResultVO().getCmplReasonCont();
        int stsfIvstSeq = obj.getSatisFactionMobileResultVO().getStsfIvstSeq();//만족도조사 일련번호
        int subyTmplSeq = obj.getSatisFactionMobileResultVO().getSubyTmplSeq();//설문 템플릿 일련번호
        ...
        surveySearchVO.setsMakCd(makCd);
        ...
        if ( StringUtil.isEmpty(stsfIvstYn) ) {
            // {설문결과}을(를) 확인하여 주세요.
        ...
        for(CampaignResultVO campaignResultVO : obj.getInsertList()){
            campaignResultVO.setRegUsrId(userId);
        ...
        resultVO.setStsfIvstSeq(stsfIvstSeq);//만족도조사 일련번호
        resultVO.setSubyTmplSeq(subyTmplSeq);//설문템플릿 일련번호
        resultVO.setEmpId(userId);//고객번호
        resultVO.setStsfIvstScoreVal(tmpTotScore); // 만족도조사 점수값
        resultVO.setStsfIvstEmpId(userId);
        resultVO.setRegUsrId(userId);
        ...
        tempVO.setEmpId(userId);//사원ID
        tempVO.setRegUsrId(userId);
    ...
    public void initJxlsContext(Context context, HMap params) throws Exception {
        ...
        if (StringUtils.isNotEmpty(sExcelTp)) {
            if ("CampaignRltMgmtList".equals(sExcelTp)) {  // 캠페인결과 관리 다운로드
                list = selectCampaignResultMgmtsByCondition(searchVO);
            }
        }

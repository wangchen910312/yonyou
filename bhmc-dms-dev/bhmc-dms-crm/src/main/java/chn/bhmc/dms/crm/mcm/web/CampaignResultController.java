...
public class CampaignResultController extends HController {
    ...
    public SearchResult selectCampaignResults(@RequestBody CampaignResultSearchVO searchVO) throws Exception {
        ...
        result.setTotal(campaignResultService.selectCampaignResultMgmtsByConditionCnt(searchVO));

        if(result.getTotal() != 0){

            List<CampaignResultVO> selectCampaignResultsByCondition = campaignResultService.selectCampaignResultMgmtsByCondition(searchVO);
    ...
    /**
     * 캠페인결과(업체평가) 데이터를 조회한다.   - 만족도 조사 설문지 및 결과 조회 [OB실행>상세팝업>만족도조사탭], [캠페인결과관리>업체평가], ]캠페인관리>결과>업체평가 ]
     * @param searchVO - 조회 조건을 포함하는 CampaignResultSearchVO
     * @return
     */
    ...
    public @ResponseBody SatisFactionManageVO selectSatisFactionSurveyResult(@RequestBody CampaignResultSearchVO searchVO) throws Exception {
        ...
        if ( campaignResultVO != null ) {
            ...
            // 캠페인 결과 설문이 있는경우 설문결과 번호를 가져온다.
            satisFactionManageVO.setStsfIvstRsltSeq(campaignResultVO.getStsfIvstRsltSeq());
     ...
     * @return
     */
    @RequestMapping(value = "/crm/mcm/campaignResult/selectCampaignResultMgmtsExcelExportCnt.do", method = RequestMethod.POST)
    @ResponseBody
    public int selectCampaignResultMgmtsByConditionCnt(@RequestBody CampaignResultSearchVO searchVO) throws Exception {

        return campaignResultService.selectCampaignResultMgmtsByConditionCnt(searchVO);

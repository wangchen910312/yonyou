...
public class SurveySatisFactionController extends HController {
    ...
    /**
     * 설문템플릿 관리 > 설문 템플릿 관리 메인 화면을 출력한다.
     * @return
     */
    ...
    public String selectSurveySatisFactionMain(Model model) throws Exception {
        ...
        //설문질문유형코드
        model.addAttribute("qestTpCdList", commonCodeService.selectCommonCodesByCmmGrpCd("CRM093", null, LocaleContextHolder.getLocale().getLanguage()));

        return "/crm/css/surveySatisFaction/selectSurveySatisFactionMain";
    }

    /**
     * 설문 템플릿 관리 메인 > 설문지명 목록 데이터를 조회한다.
    ...
    /**
     * 설문 템플릿 관리 메인 > 설문 템플릿 관리 정보를 등록/수정/삭제 한다.
    ...
    /**
     * 설문 템플릿 관리 메인 > 설문답안 목록 데이터를 조회한다.
    ...
    /**
     * 설문 템플릿 관리 메인 > 설문 템플릿 결과저장 여부를 확인한다.

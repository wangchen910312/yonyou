 ...
 * </pre>
 *
 * @ClassName   : TargetMonthAllRegVO.java
...
public class TargetMonthPartRegVO  extends AbstractExcelDataBinder {
    ...
    private String sGoalYyMm;       // 목표년월
    private String sGoalYy;         // 목표년도
    private String sGoalMm;         // 목표당월
    ...
    /**
     * @return the sGoalYyMm
     */
    public String getsGoalYyMm() {
        return sGoalYyMm;
    }
    /**
     * @param sGoalYyMm the sGoalYyMm to set
     */
    public void setsGoalYyMm(String sGoalYyMm) {
        this.sGoalYyMm = sGoalYyMm;
    }
    /**
     * @return the sGoalYy
     */
    ...
    public void bind(int rowNo, int cellNo, Object value) throws ExcelDataBindingException {
        try{
            switch(cellNo){
                ...
                case 1:this.setGoalMm(ExcelDataBinderUtil.toString(value).trim()); break;           // 목표당월
                case 2:this.setDlrCd(ExcelDataBinderUtil.toString(value).trim()); break;            // 딜러 코드
                case 3:this.setCarLineCd(ExcelDataBinderUtil.toString(value).trim()); break;        // 차종 코드
                case 4:this.setGoalPrefVal(ExcelDataBinderUtil.toString(value).trim()); break;      // 목표설정값
                default :
            }
        }catch(Exception e){
    ...
    public List<ExcelUploadError> validate(ExcelDataBindContext context) {
        ...
        // 목표 당월
        if(StringUtils.isBlank(this.getGoalMm())){

            String msg = messageSource.getMessage(
                "global.info.required.field"
                , new String[]{
                        messageSource.getMessage("mis.lbl.goalMm", null, LocaleContextHolder.getLocale())
                }
                , LocaleContextHolder.getLocale()
            );

            errors.add(new ExcelUploadError(context.getRow(), 1, this.getGoalMm(), msg));
        }
        ...
        if(StringUtils.isBlank(this.getDlrCd())){
            ...
            errors.add(new ExcelUploadError(context.getRow(), 2, this.getDlrCd(), msg));
        }
        ...
        if(StringUtils.isBlank(this.getCarLineCd())){
            ...
            errors.add(new ExcelUploadError(context.getRow(), 3, this.getCarLineCd(), msg));
        }
        ...
        if(StringUtils.isBlank(this.getGoalPrefVal())){
            ...
            errors.add(new ExcelUploadError(context.getRow(), 4, this.getGoalPrefVal(), msg));
        }

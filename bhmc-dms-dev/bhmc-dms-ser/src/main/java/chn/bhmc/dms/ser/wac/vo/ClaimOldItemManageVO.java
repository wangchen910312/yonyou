    ...
    public void bind(int rowNo, int cellNo, Object value) throws ExcelDataBindingException {
        try{
            switch(cellNo){
                ...
                case 3:this.setCollcQty(ExcelDataBinderUtil.getDoubleValue(value)); break;    //회수수량
            }
        }catch(Exception e){
    ...
    public List<ExcelUploadError> validate(ExcelDataBindContext context) {
        ...
        if(StringUtils.isBlank(this.getDlrCd())){
            ...
            String msg = messageSource.getMessage(
                ...
                , new String[]{
                        messageSource.getMessage("ser.lbl.dealerCo", null, LocaleContextHolder.getLocale())
                }
        ...
        if(StringUtils.isBlank(this.getClaimDocNo())){
            ...
            String msg = messageSource.getMessage(
                "global.info.required.field"
                , new String[]{
                        messageSource.getMessage("ser.lbl.claimNo", null, LocaleContextHolder.getLocale())
                }
                , LocaleContextHolder.getLocale()
            );
        ...
        if(StringUtils.isBlank(this.getItemCd())){
            ...
            String msg = messageSource.getMessage(
                "global.info.required.field"
                , new String[]{
                        messageSource.getMessage("ser.lbl.oitemCd", null, LocaleContextHolder.getLocale())
                }
                , LocaleContextHolder.getLocale()
            );

            errors.add(new ExcelUploadError(context.getRow(), 3, this.getItemCd(), msg));
        }

        //회수수량
        if(this.getCollcQty()==0||this.getCollcQty().equals("")){

            String msg = messageSource.getMessage(
                "global.info.required.field"
                , new String[]{
                        messageSource.getMessage("ser.lbl.recQty", null, LocaleContextHolder.getLocale())
                }
                , LocaleContextHolder.getLocale()
            );

            errors.add(new ExcelUploadError(context.getRow(), 4, this.getCollcQty().toString(), msg));
        }

    ...
    function Resumable(opt) {
      ...
      this.defaults = {
        ...
        maxFilesErrorCallback: function(files, errorCount) {
          var maxFiles, _ref;
          maxFiles = that.getOpt('maxFiles');
          //return alert('Please upload ' + maxFiles + ' file' + ((maxFiles != null && maxFiles === 1)? '' : 's') + ' at a time.');
          //최대 업로드 파일 갯수는 1개 입니다.
          return dms.notification.info('最大文件录入个数是' + maxFiles + '个。');
        },
        minFileSize: void 0,
        minFileSizeErrorCallback: function(file, errorCount) {
          //return alert(file.fileName+' is too small, please upload files larger than ' + that.formatSize(that.getOpt('minFileSize')) + '.');
          //파일 사이즈가 너무 작습니다. 업로드 파일은 1M 이상 이어야 합니다.
          return dms.notification.info('文件太小, 上传文件最小为' + that.formatSize(that.getOpt('minFileSize')) + '。');
        },
        maxFileSize: void 0,
        maxFileSizeErrorCallback: function(file, errorCount) {
          //return alert(file.fileName+' is too large, please upload files less than ' + that.formatSize(that.getOpt('maxFileSize')) + '.');
          //파일 사이즈가 너무 큽니다. 업로드 파일은 1M 이하 이어야 합니다.
          return dms.notification.info('文件太大, 上传文件最大为' + that.formatSize(that.getOpt('maxFileSize')) + '。');
        },
		fileType: ["7z","alz","doc","docx","gif","jpg","jpeg","pdf","png","ppt","pptx","text","txt","xls","xlsx","zip","avi","wmv","mpeg","mpg","mkv","mp4","tp","ts","mov","swf","flv","mp3","rar","png","sql"],
		fileTypeErrorCallback: function(file, errorCount) {
			//alert(file.fileName||file.name +' has type not allowed, please upload files of type ' + $.getOpt('fileType') + '.');
		    //선택한 파일은 업로드 할 수 없습니다. 업로드 가능한 파일 유형[xlx, pptx]
			dms.notification.info('无法上传选择的文件。 只能上传 [' + that.getOpt('fileType') + '] 文件。');
		}
      };
      if (this.opt == null) {
        this.opt = {};
...
//      errorCount = 0;
//      _ref = this.getOpt(['maxFiles', 'minFileSize', 'maxFileSize', 'maxFilesErrorCallback', 'minFileSizeErrorCallback', 'maxFileSizeErrorCallback']),

        var errorCount, file, files, maxFileSize, maxFileSizeErrorCallback, maxFiles, maxFilesErrorCallback, minFileSize, minFileSizeErrorCallback, resumableFile, _i, _len;
        //console.log("appendFilesFromFileList");
        errorCount = 0;
        maxFiles = this.getOpt('maxFiles');
        minFileSize = this.getOpt('minFileSize');
        maxFileSize = this.getOpt('maxFileSize');
        maxFilesErrorCallback = this.getOpt('maxFilesErrorCallback');
        minFileSizeErrorCallback = this.getOpt('minFileSizeErrorCallback');
        maxFileSizeErrorCallback = this.getOpt('maxFileSizeErrorCallback');
		fileType = this.getOpt('fileType');
		fileTypeErrorCallback = this.getOpt('fileTypeErrorCallback');
      ...
      for (_i = 0, _len = fileList.length; _i < _len; _i++) {
		...
		if(fileType.length > 0){
          ...
          for(var index in fileType){
            var extension = '.' + fileType[index];
            if(file.name.toLowerCase().indexOf(extension, file.name.length - extension.length) !== -1){
              fileTypeFound = true;
              break;
            }
          }
  ...
  window.ResumableChunk = ResumableChunk = (function() {
    ...
    function ResumableChunk(resumableObj, fileObj, offset, callback) {
      ...
      this.xhr = null;
      this.uniqueIdentifier = fileObj.uniqueIdentifier;
    }
    ...
    ResumableChunk.prototype.test = function() {
      ...
      this.pushParams(params, 'resumableFileContentType', this.fileObj.contentType);
    ...
    ResumableChunk.prototype.send = function() {
      ...
      query = {
        ...
        resumableFileContentType: this.fileObj.contentType,
  ...
  window.ResumableFile = ResumableFile = (function() {
    ...
    ResumableFile.prototype.progress = function() {
      ...
      //console.log("ret:" + ret);
      //console.log("_prevProgress:" + this._prevProgress);

...
//ipad 텍스트 입력되지 않는 목록입력     ex)column_name : grid_id
var popupCellJson = {"custReqCont" : "preChkForm"
                    ,"remark" : "preChkForm"};
var popupCellColumnNm = "";
var _focusTimeout;
...
$(document).on('focus','input, textarea', function() {
  //khskhs 문제시 삭제 : 태블릿 키보드 안가라게 스크롤하는거 textArea, iPad 용 코드 추가 필요 할 것 같음
    try{
        ...
        {
            /************************************************************************
              <ipad 텍스트 입력되지 않는 버그 대응>
              팝업, 그리드, row, cellIndex, cell인덱스, 팝업명, dataItem에 대한 데이터 할당
              팝업생성 및 제목/내용/컬럼 삽입
             ************************************************************************/
            if(popupCellJson[this.name] == "preChkForm"){
                var win = $("#window_sa").data("kendoWindow");
                var grid = $(this).closest(".k-grid").data("kendoExtGrid");
                var tr = $(this).closest("tr")[0];
                var td = $(this).closest("td")[0];
                var idx = td.cellIndex;
                var title = grid.columns[idx].title;
                ...
                $("#window_sa_wnd_title").text(title);
                $("#open_saInput_textarea").val(dataItem[this.name]);
                popupCellColumnNm = this.name;
            }
        }
        else
        {

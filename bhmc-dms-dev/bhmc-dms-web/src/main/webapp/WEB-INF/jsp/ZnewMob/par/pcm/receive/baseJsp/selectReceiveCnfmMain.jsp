...
<section id="content">
    ...
    <!-- content_list -->
    <c:import url="/WEB-INF/jsp/ZnewMob/par/pcm/receive/listJsp/selectReceiveCnfmList.jsp" />
    <!-- content_search -->
            ...
            <table class="flist01">
                ...
                <tbody>
                    ...
                    <tr>
                         ...
                         <td class="f_term">
                             <div class="f_item01">
                                <input type="date" id="sInvcDtFr" style="width:140px; background-color: #ffffff;"/>
                             ...
                             <div class="f_item01">
                                 <input type="date" id="sInvcDtTo" style="width:140px; background-color: #ffffff;"/>
    ...
    <div class="content_right" >
        <c:import url="/WEB-INF/jsp/ZnewMob/par/pcm/receive/listJsp/selectReceiveCnfmItemList.jsp" />
    </div>
...
function getBarcodeNum(code){ // js (앱에서 리턴되는 값 받는 스크립트)
    $("input[name='isSave']").each(function(cnt){
        if(code == $("span[name='itemCd']").text()){
            contentDetail3(code);
        }
    });
};
    ...
    setTimeout(function(){
        $("input[name='isSave']").each(function(cnt){
            for(var i=0; i<saveList.length; i++){
                var _key = $("span[name='itemCd']").text();
                if(_key == saveList[i].itemCd){
                    $("input[name='isSave']")[cnt].checked = true;
                }
            }
        });
    }, 200);
...
var setCheckItemList2 = function(){
    setTimeout(function(){
        $("input[name='isSave']").each(function(cnt){
            for(var i=0; i<saveList.length; i++){
                var code = $("span[name='itemCd']").text();
...
$(document).ready(function() {
  ...
  //버튼- 초기화
    ...
    $("#listView").height($(window).height()-($("#header").outerHeight()+$("#content_list > div").outerHeight()+$(".listhead").outerHeight()+$(".btnfixed").outerHeight()+10));
...
function doSave(){
    ...
    setTimeout(function(){
        $("input[name='isSave']").each(function(cnt){
            var _key = $("input[name='purcOrdNo']").val();
...
function contentItemList(dataItem) {
    ...
    listData2.read();
    saveList = [];
    $("#listView2").height($(window).height()-($("#header").outerHeight()+$("#content_itemList > div").outerHeight()+$("#content_itemList .listhead").outerHeight()+$(".btnfixed").outerHeight()));
}

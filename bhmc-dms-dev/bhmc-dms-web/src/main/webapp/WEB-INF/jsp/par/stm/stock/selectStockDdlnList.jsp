    ...
    <section class="group">
        ...
        <div id="tabstrip" class="tab_area mt10">
            <ul>
                ...
                <li id="grGiStatus" ><spring:message code="par.title.receiveStockReport" /><!-- 입고재고현황 --><!-- 입고재고현황 --></li>
            </ul>
            ...
            <div>
                <div  class="table_grid">
                    <!-- 입고재고현황 그리드 -->
                    <div id="gridStatus" class="resizable_grid"></div>
                </div>
            </div>
        </div>
    </section>
...
<script>
    ...
    <c:forEach var="obj" items="${storageList}" varStatus="status">
        //완성차창고 : 02, 용품창고 : 06, 판매용품창고 : 20 제외(dropdownlist) , 용품창고도 보여주기로 함. 요청자 : 용우(Howee微) 2018.05.28
        //if("${obj.strgeTp}" !== '02' && "${obj.strgeTp}" !== '06' && "${obj.strgeTp}" !== '20'){
        if("${obj.strgeTp}" !== '02' && "${obj.strgeTp}" !== '20'){
      ...
      //창고선택
        ...
        $("#btnExcelDownload").kendoButton({
            click: function(e){
                ...
                } else if(selectTabId == "stockDdlnDtl") {
                    beanName = "stockDdlnService";
                    templateFile = "StockDdlnDetail_Tpl.xlsx";
                    fileName = "StockDdlnDetail.xlsx";
                } else if(selectTabId == "grGiStatus"){
                    beanName = "stockDdlnService";
                    templateFile = "stockGrGiStatus_Tpl.xlsx";
                    fileName = "stockGrGiStatus.xlsx";
                }
                dms.ajax.excelExport({
                    ...
                    ,"sStrgeCd": $("#sStrgeCd").data("kendoExtDropDownList").value().trim()
                    ,"sLocCd": $("#sLocCd").val()
        ...
        $("#btnSearch").kendoButton({
            click: function(e){
                ...
                if(dms.string.isEmpty($("#sStrgeCd"))){
                    // 딜러를 선택하십시오
                ...
                $("#gridStatus").data("kendoExtGrid").dataSource.read();
            }
        });

        // 입고 헤더 그리드
        $("#grid").kendoExtGrid({
            gridId:"G-PAR-0519-184807"
           ,height: 450
           ,dataSource:{
                transport:{
                    read:{
                ...
                ,schema:{
                    ...
                    ,model:{
                      ...
                      ,fields:{
                          ...
                          , sntMmDdlnStockQty :{ type:"number", editable: false } //당월마감재고수량
                          , psntMmDdlnStockPrc :{ type:"number", editable: false } //당월마감이동평균가격
                          , calcPsntMmDdlnStockPrc :{ type:"number", editable: false } //당월마감이동평균가격
                          , psntMmDdlnStockAmt :{ type:"number", editable: false } //당월마감재고금액
                          , calcPsntMmDdlnStockAmt :{ type:"number", editable: false } //당월마감재고금액
                          ...
                          , currentMovingAvgPrc:{ type:"number", editable: false } //이동평균단가(현재)
                          , grMovingAvgPrc       :{ type:"number", editable: false } //입고이동평균단가
                          , grMovingAvgTaxDdctPrc:{ type:"number", editable: false } //입고이동평균단가(세금제외)
                          , grMovingAvgAmt       :{ type:"number", editable: false } //입고이동평균금액
                          , grMovingAvgTaxDdctAmt:{ type:"number", editable: false } //입고이동평균금액(세금제외)
                          , giMovingAvgPrc       :{ type:"number", editable: false } //출고이동평균단가
                          , giMovingAvgTaxDdctPrc:{ type:"number", editable: false } //출고이동평균단가(세금제외)
                          , giMovingAvgTaxDdctAmt:{ type:"number", editable: false } //출고이동평균금액(세금제외)
                          ...
                          , obtGiQty           :{ type:"number", editable: false } //수량
                ...
                ,aggregate:[
                            { field:"preMmDdlnStockQty", aggregate:"sum" }
                            ,{ field:"preMmDdlnStockAmt", aggregate:"sum" }
                            ,{ field:"psntMmDdlnStockQty", aggregate:"sum" }
                            ,{ field:"psntMmDdlnStockAmt", aggregate:"sum" }
                            ,{ field:"grQty", aggregate:"sum" }
                            ,{ field:"grAmt", aggregate:"sum" }
                            ,{ field:"giQty", aggregate:"sum" }
                            ,{ field:"giBaseAmt", aggregate:"sum" }
                            ,{ field:"grMovingAvgAmt", aggregate:"sum" }
                            ,{ field:"grMovingAvgTaxDdctAmt", aggregate:"sum" }
                            ,{ field:"giMovingAvgTaxDdctAmt", aggregate:"sum" }
                            ,{ field:"giAmt", aggregate:"sum" }
                            ,{ field:"purcGrQty", aggregate:"sum" }
                            ,{ field:"purcGrAmt", aggregate:"sum" }
                            ,{ field:"obtGrQty", aggregate:"sum" }
                            ,{ field:"obtGrAmt", aggregate:"sum" }
                            ,{ field:"palGrQty", aggregate:"sum" }
                            ,{ field:"palGrAmt", aggregate:"sum" }
                            ,{ field:"strgeMoveGrQty", aggregate:"sum" }
                            ,{ field:"strgeMoveGrAmt", aggregate:"sum" }
                            ,{ field:"etcGrQty", aggregate:"sum" }
                            ,{ field:"etcGrAmt", aggregate:"sum" }
                            ,{ field:"ctSaleQty", aggregate:"sum" }
                            ,{ field:"ctSaleBaseAmt", aggregate:"sum" }
                            ,{ field:"ctSaleAmt", aggregate:"sum" }
                            ,{ field:"wksSaleQty", aggregate:"sum" }
                            ,{ field:"wksSaleBaseAmt", aggregate:"sum" }
                            ,{ field:"wksSaleAmt", aggregate:"sum" }
                            ,{ field:"serSaleQty", aggregate:"sum" }
                            ,{ field:"serSaleBaseAmt", aggregate:"sum" }
                            ,{ field:"serSaleAmt", aggregate:"sum" }
                            ,{ field:"parSaleQty", aggregate:"sum" }
                            ,{ field:"parSaleBaseAmt", aggregate:"sum" }
                            ,{ field:"parSaleAmt", aggregate:"sum" }
                            ,{ field:"strgeMoveGiQty", aggregate:"sum" }
                            ,{ field:"strgeMoveGiBaseAmt", aggregate:"sum" }
                            ,{ field:"intRcvGiQty", aggregate:"sum" }
                            ,{ field:"intRcvGiBaseAmt", aggregate:"sum" }
                            ,{ field:"intRcvGiAmt", aggregate:"sum" }
                            ,{ field:"obtGiQty", aggregate:"sum" }
                            ,{ field:"obtGiBaseAmt", aggregate:"sum" }
                            ,{ field:"obtGiAmt", aggregate:"sum" }
                            ,{ field:"lossGiQty", aggregate:"sum" }
                            ,{ field:"lossGiBaseAmt", aggregate:"sum" }
                            ,{ field:"etcGiQty", aggregate:"sum" }
                            ,{ field:"etcGiBaseAmt", aggregate:"sum" }
                            ,{ field:"etcGiAmt", aggregate:"sum" }
                        ]
            }
            , selectable:"row"
            , scrollable :true
            ...
            , columns:[
                ...
                , {//로케이션
                 ...
                 , {//입고이동단가
                     field      :"grMovingAvgPrc"
                    ,title      :"<spring:message code='par.lbl.receive' /><spring:message code='par.lbl.stockCost' />"
                    ,attributes :{"class":"ar"}
                    ,format     :"{0:n2}"
                    ,width      :100
                 }
                 , {//입고이동단가(세금제외)
                     field      :"grMovingAvgTaxDdctPrc"
                    ,title      :"<spring:message code='par.lbl.receive' /><spring:message code='par.lbl.stockCost' />(<spring:message code='par.lbl.taxSprt' />)"
                    ,attributes :{"class":"ar"}
                    ,format     :"{0:n2}"
                    ,width      :100
                 }
                 , {//입고이동금액
                     field      :"grMovingAvgAmt"
                    ,title      :"<spring:message code='par.lbl.receive' /><spring:message code='par.lbl.movingAvgCost' />"
                    ,attributes :{"class":"ar"}
                    ,format     :"{0:n2}"
                    ,aggregates :["sum"]
                    ,footerTemplate:"<div class='ar'>#= setSumData(sum)#</div>"
                 ...
                 , {//입고이동금액(세금제외)
                     field      :"grMovingAvgTaxDdctAmt"
                    ,title      :"<spring:message code='par.lbl.receive' /><spring:message code='par.lbl.movingAvgCost' />(<spring:message code='par.lbl.taxSprt' />)"
                    ,attributes :{"class":"ar"}
                    ,format     :"{0:n2}"
                    ,aggregates :["sum"]
                    ,footerTemplate:"<div class='ar'>#= setSumData(sum)#</div>"
                 ...
                 , {//출고이동단가
                     field      :"giMovingAvgPrc"
                    ,title      :"<spring:message code='par.lbl.issue' /><spring:message code='par.lbl.stockCost' />"
                    ,attributes :{"class":"ar"}
                    ,format     :"{0:n2}"
                    ,width      :100
                 }
                 , {//출고이동단가(세금제외)
                     field      :"giMovingAvgTaxDdctPrc"
                    ,title      :"<spring:message code='par.lbl.issue' /><spring:message code='par.lbl.stockCost' />(<spring:message code='par.lbl.taxSprt' />)"
                    ,attributes :{"class":"ar"}
                    ,format     :"{0:n2}"
                    ,width      :100
                 }
                 ...
                 , {//출고이동금액(세금제외)
                     field      :"giMovingAvgTaxDdctAmt"
                    ,title      :"<spring:message code='par.lbl.issue' /><spring:message code='par.lbl.movingAvgCost' />(<spring:message code='par.lbl.taxSprt' />)"
                    ,attributes :{"class":"ar"}
                    ,format     :"{0:n2}"
                    ,aggregates:["sum"]
                    ,footerTemplate:"<div class='ar'>#= setSumData(sum)#</div>"
                 ...
                 , {//이동단가(현재)
                     field      :"currentMovingAvgPrc"
                    ,title      :"<spring:message code='par.lbl.movingAvgPrc' />(当前)"
                    ,attributes :{"class":"ar"}
                    ,format     :"{0:n2}"
                    ,width      :100
                    ,hidden     :true
        ...
        $("#gridDetail").kendoExtGrid({
            gridId:"G-PAR-0519-184704"
           ,height: 450
           ,dataSource:{
                transport:{
                    read:{
                        url:"<c:url value='/par/stm/stock/selectStockDdlnByConditionByItemDistCd.do' />"
                    }
                    , parameterMap:function(options, operation) {
                        if (operation === "read") {
                    ...
                    ,model:{
                       id:"itemCd"
                           ,fields:{
                             ...
                             , preMmDdlnStockAmt  :{ type:"number", editable: false } //전월마감재고금액
                             , psntMmDdlnStockQty :{ type:"number", editable: false } //당월마감재고수량
                             , calcPsntMmDdlnStockQty :{ type:"number", editable: false } //당월마감재고수량
                             , psntMmDdlnStockPrc :{ type:"number", editable: false } //당월마감이동평균가격
                             , calcPsntMmDdlnStockPrc :{ type:"number", editable: false } //당월마감이동평균가격
                             , psntMmDdlnStockAmt :{ type:"number", editable: false } //당월마감재고금액
                             , calcPsntMmDdlnStockAmt :{ type:"number", editable: false } //당월마감재고금액
                ...
                ,aggregate:[
                    ...
                    ,{ field:"preMmDdlnStockAmt", aggregate:"sum" }
                    ,{ field:"psntMmDdlnStockQty", aggregate:"sum" }
                    ,{ field:"calcPsntMmDdlnStockQty", aggregate:"sum" }
                    ,{ field:"psntMmDdlnStockAmt", aggregate:"sum" }
                    ,{ field:"calcPsntMmDdlnStockAmt", aggregate:"sum" }
                    ,{ field:"grQty", aggregate:"sum" }
                    ,{ field:"grAmt", aggregate:"sum" }
                    ,{ field:"giQty", aggregate:"sum" }
                    ,{ field:"giBaseAmt", aggregate:"sum" }
            ...
            , columns:[
                ...
                , {   //품목구분
                 ...
                 , {//출고이동금액(세금제외)
                     field      :"giMovingAvgTaxDdctAmt"
                    ,title      :"<spring:message code='par.giMoveAmt' />(<spring:message code='par.lbl.taxSprt' />)"
                    ,attributes :{"class":"ar"}
                    ,format     :"{0:n2}"
                    ,aggregates:["sum"]
                    ,footerTemplate:"<div class='ar'>#= setSumData(sum)#</div>"
        ...
        //입고재고현황 그리드
        $("#gridStatus").kendoExtGrid({
            gridId:"G-PAR-0520-171802"
            ,height: 450
            ,dataSource: {
                transport: {
                    read:{
                        url:"<c:url value='/par/stm/stock/selectStockDdlnGrGiByCondition.do' />"
                    }
                    ,parameterMap:function(options, operation) {
                        if(operation === "read") {
                            var params = {};
                ...
                ,schema: {
                    ...
                    ,model:{
                        id:"strgeCd"
                            ,fields:{
                                dlrCd            :{ type:"string", editable: false }  // (컬럼에 없음)
                            , strgeCd            :{ type:"string", editable: false } //창고
                            , yyMm               :{ type:"string", editable: false } //년월
                            , preMmDdlnStockQty  :{ type:"number", editable: false } //전월마감재고수량
                            , preMmDdlnStockAmt  :{ type:"number", editable: false } //전월마감재고금액
                            , psntMmDdlnStockQty :{ type:"number", editable: false } //당월마감재고수량 (컬럼에 없음)
                            , calcPsntMmDdlnStockQty :{ type:"number", editable: false } //당월마감재고수량
                            , psntMmDdlnStockAmt :{ type:"number", editable: false } //당월마감재고금액 (컬럼에 없음)
                            , calcPsntMmDdlnStockAmt :{ type:"number", editable: false } //당월마감재고금액
                            , grQty              :{ type:"number", editable: false } //입고수량
                            , grAmt              :{ type:"number", editable: false } //입고금액
                            , giQty              :{ type:"number", editable: false } //출고수량
                            ...
                            , etcGrQty           :{ type:"number", editable: false } //기타입고수량
                            , etcGrAmt           :{ type:"number", editable: false } //기타입고금액
                            , ctSaleQty          :{ type:"number", editable: false } //COUNTER판매수량  (컬럼에 없음)
                            , ctSaleBaseAmt      :{ type:"number", editable: false } //COUNTER판매원가금액    (컬럼에 없음)
                            , ctSaleAmt          :{ type:"number", editable: false } //COUNTER판매금액  (컬럼에 없음)
                            , wksSaleQty         :{ type:"number", editable: false } //WORKSHOP판매수량 (컬럼에 없음)
                            , wksSaleBaseAmt     :{ type:"number", editable: false } //WORKSHOP판매원가금액   (컬럼에 없음)
                            , wksSaleAmt         :{ type:"number", editable: false } //WORKSHOP판매금액 (컬럼에 없음)
                            , serSaleQty         :{ type:"number", editable: false } //정비판매수량
                            , serSaleBaseAmt     :{ type:"number", editable: false } //정비판매원가금액
                            , serSaleAmt         :{ type:"number", editable: false } //정비판매금액
                            , parSaleQty         :{ type:"number", editable: false } //부품판매수량
  ...
  //부품 팝업 열기 함수.
    ...
    function initPage(){
        ...
        $("#gridStatus").data("kendoExtGrid").dataSource.data([]);
        ...
        $("#sAccYyMmTo").data("kendoExtMaskedDatePicker").value(nowYYMM);
        $("#sAccYyMmFr").data("kendoExtMaskedDatePicker").max(new Date(toYY,toMM+1));
        $("#sAccYyMmTo").data("kendoExtMaskedDatePicker").min(new Date(nowYY,nowMM-1));
        $("#sAccYyMmTo").data("kendoExtMaskedDatePicker").max(new Date(nowYY,nowMM));
    }
         ...
         //INVOICE입고일
           if(id === 'sAccYyMmFr'){
               ...
               var maxDate = new Date(toDt);
                   toYY = maxDate.getFullYear();
                   toMM = maxDate.getMonth() + 1;
                   $("#sAccYyMmTo").data("kendoExtMaskedDatePicker").value(maxToYY + '-' + dms.string.lpad((maxToMM+1) +'','0',2));

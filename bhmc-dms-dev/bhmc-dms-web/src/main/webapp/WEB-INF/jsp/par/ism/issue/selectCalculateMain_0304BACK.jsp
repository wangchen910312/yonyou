                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="ar"><input id="parPointUseAmt" data-type="number" min="0" class="form_numeric ar form_readonly" readonly/></td>
  ...
  //영수증유형 Array
    ...
    <c:forEach var="obj" items="${ordStatCdList}" varStatus="status">
        if(ordStatChkObj.hasOwnProperty("${obj.cmmCd}")){
            if("${obj.cmmCd}" === '07'){
                ordStatCdList.push({"cmmCd":"${obj.cmmCd}", "cmmCdNm":"退货", "useYn":"${obj.useYn}"});
            }else{
        ...
        if("${obj.cmmCd}" === '01'){
            ordStatCdObj["${obj.cmmCd}"] = "登记";
        }else if("${obj.cmmCd}" === '02'){
            ordStatCdObj["${obj.cmmCd}"] = "确认";
        }else if("${obj.cmmCd}" === '05'){
            ordStatCdObj["${obj.cmmCd}"] = "付款";
        }else if("${obj.cmmCd}" === '07'){
            ordStatCdObj["${obj.cmmCd}"] = "退货";
        }else{
            ordStatCdObj["${obj.cmmCd}"] = "${obj.cmmCdNm}";
        }
        if("${obj.cmmCd}" === '01'){
            ordItemStatCdObj["${obj.cmmCd}"] = "登记";
        }else if("${obj.cmmCd}" === '07'){
            ordItemStatCdObj["${obj.cmmCd}"] = "退货";
        }else{
            ordItemStatCdObj["${obj.cmmCd}"] = "出库";
        }
    </c:forEach>
  ...
  //블루멤버쉽 등급 코드
    ...
    $(document).ready(function() {
      ...
      //오더상태선택
        ...
        $("#parPointUseAmt").kendoExtNumericTextBox({
            format:"n2"
           ,spinners:false
        });
     ...
     // 인쇄
        $("#btnPrint").kendoButton({
            click:function(e){
                if(dms.string.isEmpty($("#parSaleOrdNo").val())){
                    // 판매번호를 선택하십시오
                    dms.notification.warning("<spring:message code='par.lbl.partsSaleNo' var='partsSaleNo' /><spring:message code='global.info.chkSelectItemMsg' arguments='${partsSaleNo}'/>");
                    return false;
                }
                // ready message
                parent._createOrReloadTabMenu("<spring:message code='par.title.saleCalculatePrintDoc' />", "<c:url value='/ReportServer?reportlet=par/selectPartsSaleCalcPrintMain.cpt'/>&sDlrCd=${dlrCd}&sParSaleOrdNo="+$("#parSaleOrdNo").val());
            }
        });
       ...
       // 분리 팝업
        ...
        $("#btnCancel").kendoButton({
            click:function(e){
                ...
                if(!dms.string.isEmpty(cnclCalcObj)){
                    $.ajax({
                        ...
                        ,success:function(result, textStatus){
                           // dms.notification.success("<spring:message code='global.info.success'/>");
                            ...
                            updateParam["parSaleOrdNo"]  = $("#parSaleOrdNo").val();
                            updateParam["calcDocNo"]     = '';
                            updateParam["ordStatCd"]     = '02';

                            $.ajax({
                                url:"<c:url value='/par/ism/issue/updateCalculateInfo.do' />"
          ...
          //bmCancel();
            ...
            if(calcData != null){
                $.each(calcData, function(i,v){
                    if(this.paymTp == "04"){
                         paymAmt = Number(this.paymAmt - reduceCost);
                        if(paymAmt < 0) {
                            $("#etcBmPointUseAmt").val(Number(this.paymAmt));
     ...
     // 딜러포인트 취소
        dealerCancel = function(){
            ...
            $.each(calcData, function(i,v){
                if(this.paymTp == "04"){
                    this.calcAmt = (Number(this.calcAmt) + Number(parPointUseAmt)).toFixed(2);
                    this.paymAmt = (Number(this.paymAmt) + Number(parPointUseAmt)).toFixed(2);
                    this.dcPermNm = "";
                    this.dcPermCd = "";
                }
            });
            ...
            if(calcData != null){
                $.each(calcData, function(i,v){
                    if(calcData[i].paymTp == "04"){
                        occrPointVal = rowData.occrPointVal * Number($("#pointExchgScale").val()).toFixed(1);
                        ...
                        if(paymAmt < 0) {
                            ...
                            $("#parPointUseAmt").data("kendoExtNumericTextBox").value(Number(occrPointVal));
                            fnLastCalcAmt();   // 최종정산금액 계산
                            ...
                            calcData[i].paymAmt = Number($("#parCalcSumAmt").data("kendoExtNumericTextBox").value() + Number(wonUnitCutAmt) - Number(demicPointCutAmt)).toFixed(2);
                        }
                    }
                });
                calcGrid.refresh();
       ...
       // BM포인트 셋팅
        setBmPointData = function(){
            ...
            if(calcData != null){
                $.each(calcData, function(i,v){
                    if(this.paymTp == "04"){
                         paymAmt = Number(this.paymAmt - cosumePoints);
                        ...
                        if(paymAmt < 0) {
                            ...
                            fnLastCalcAmt();   // 최종정산금액 계산
                            this.calcAmt = (Number(this.calcAmt) - Number(cosumePoints)).toFixed(2);
                            this.paymAmt = (Number(this.paymAmt) - Number(cosumePoints)).toFixed(2);
                        }
                    }
                });
                calcGrid.refresh();
     ...
     // 블루멤버스 취소setDlrPointData
        bmCancel = function(pType){
            ...
            $.each(calcData, function(i,v){
                if(this.paymTp == "04"){
                    this.calcAmt = (Number(this.calcAmt) + Number(cancelBmPointTotAmt)).toFixed(2);
                    this.paymAmt = (Number(this.paymAmt) + Number(cancelBmPointTotAmt)).toFixed(2);
                    this.dcPermNm = "";
                    this.dcPermCd = "";
                }
            });
     ...
     // 기타할인
        setEtcDcAmt = function() {
            ...
            if(rowData != null){
                ...
                parTotAmt = Number(parCalcSumAmt) - Number(rowData.parDcAmt);
                ...
                $.each(calcData, function(i,v){
                    if(this.paymTp == "04"){
                        this.calcAmt = (Number(this.calcAmt) - Number(parEtcDcAmt)).toFixed(2);
                        this.paymAmt = (Number(this.paymAmt) - Number(parEtcDcAmt)).toFixed(2);
                    }
                });
                calcGrid.refresh();
            }
        ...
        *************************************************************/
        $("#gridHeader").kendoExtGrid({
                ...
                if(rowLen === 1){
                    $.each(rows, function(idx, row){
                        ...
                        if( dataItem != null && dataItem != 'undefined'){
                            if(dataItem.parSaleOrdNo == $("#parSaleOrdNo").val()){
            ...
            ,columns:[
                ...
                ,{field      :"parPointUseAmt"   ,hidden     :true}//딜러포인트사용금액
        ...
        $("#gridDetail").kendoExtGrid({
                ...
                ,schema:{
                    ...
                    , model:{
                        ...
                        ,fields:{
                            ...
                            , expcParAmt        :{ type:"number", editable:false } //부품비용
                            , dcRate            :{ type:"number", editable:true } //할인율
                            , dcAmt             :{ type:"number", editable:false } //할인금액
            ...
            ,columns:[
                ...
                , {   //부품판매상태
                 ...
                 ,{field:"giTp", title:"<spring:message code='global.lbl.giTp' />", width:70}
                 ,{//수량
                     field      :"itemQty"
                 ...
                 ,{field:"partCost", title:"<spring:message code='par.lbl.partsSaleAmt' />", width:90,format:"{0:n2}", attributes:{"class":"ar"}
                 }                     // 예정부품대
                 ...
                 ,{field:"dcAmt",title:"<spring:message code='par.lbl.cstDcAmt' />",width:90
                    ,attributes:{ "style":"text-align:right"} ,format:"{0:n2}"}//할인금액
                 ,{field:"expcParAmt",title:"<spring:message code='par.lbl.partSaleCost' />"
                    ,width:90,attributes:{ "style":"text-align:right"},format:"{0:n2}"}//부품비용
        ...
        $("#calcGrid").kendoExtGrid({
            ...
            ,columns:[
                ...
                ,{field:"dcPermNm", title:"<spring:message code='ser.lbl.dcAuthor' />", width:80, attributes:{"class":"ac"}
                }//할인권한
    ...
    function setDcData(jsonObj){
        ...
        $.each(partDatas,function(i,v){
            ...
            if(dcTp == "01"){
                if(parDcRate > partDatas[i].dcRate){

                    //판매확정 시나 판매정산시
                    parCalAmt = Number((partDatas[i].itemPrc * partDatas[i].itemQty));
                    dcCalAmt  = Number(parDcAmt);
                    parTotAmt = Number(((partDatas[i].itemPrc * partDatas[i].itemQty) - parDcAmt).toFixed(2));
                ...
                if(parDcRate > partDatas[i].dcRate){
                    parCalAmt = Number((partDatas[i].itemPrc * partDatas[i].itemQty));
                    dcCalRate =  ((parDcRate / 100) ).toFixed(2); // 할인율
                    parTotAmt = Number((parCalAmt * (1-dcCalRate)).toFixed(2));
                    dcCalAmt  = Number(((partDatas[i].itemPrc * partDatas[i].itemQty) - ((parCalAmt * (1-dcCalRate)).toFixed(2))).toFixed(2));
        ...
        $("#viewParDcRate").data("kendoExtNumericTextBox").value(dcRateTot * 100);
        ...
        $.each(datas,function(i,v){
            if(datas[i].paymTp == "04"){
                ...
                datas[i].paymAmt = Number(Number($("#parCalcSumAmt").data("kendoExtNumericTextBox").value())  + Number(datas[i].wonUnitCutAmt) - Number(datas[i].demicPointCutAmt));
                datas[i].dcPermNm = "";
                datas[i].dcPermCd = "";
            }
        });
    ...
    function dcRoleCancel(){
        var partData = $("#gridDetail").data("kendoExtGrid").dataSource.data(),
        ...
        $.each(calcData, function(i,v){
            if(this.paymTp == "04"){
                this.paymAmt = (Number(this.paymAmt)  + Number(parDcAmt)).toFixed(2);
                this.calcAmt = (Number(this.calcAmt)  + Number(parDcAmt)).toFixed(2);
                this.dcPermNm = "";
                this.dcPermCd = "";
            }
        });
    ...
    function fnLastCalcAmt(){
        ...
        var parWhDcAmt       = $("#parWhDcAmt").data("kendoExtNumericTextBox").value(),
            parBmPointUseAmt = $("#parBmPointUseAmt").data("kendoExtNumericTextBox").value(),
            parPointUseAmt   = $("#parPointUseAmt").data("kendoExtNumericTextBox").value(),
            parEtcDcAmt      = $("#parEtcDcAmt").data("kendoExtNumericTextBox").value();

        $("#parCalcSumAmt").data("kendoExtNumericTextBox").value(Number(parWhDcAmt - parEtcDcAmt - parBmPointUseAmt - parPointUseAmt));
    };
    ...
    function fnCalcConfirm(status){
        ...
        if(status === '04'){
          ...
          //정산 확정 전 재 저장 시 신규 판매금액으로 재 셋팅한다.
            if(!dms.string.isEmpty($("#calcDocNo").val()) && (Number(compareParCalcAmt) !== Number(parCalcAmt))){
                ...
                $("#parPointUseAmt").data("kendoExtNumericTextBox").value(0);//selectedItem.parPointUseAmt);
        ...
        formParam.parPointUseAmt   = Number($("#parPointUseAmt").data("kendoExtNumericTextBox").value());
        ...
        $.ajax({
            ...
            ,success:function(result, textStatus){
                ...
                if(!dms.string.isEmpty($("#calcDocNo").val())){
                    ...
                    updateParam["parSaleOrdNo"]  = $("#parSaleOrdNo").val();
                    updateParam["calcDocNo"]     = $("#calcDocNo").val();
                    if(status === '03'){
                        updateParam["ordStatCd"]     = '02';
                    }else if(status === '04'){
                        updateParam["ordStatCd"]     = '04';
                    }
    ...
    function gridHeaderChanged(selectedItem){
        ...
        if( selectedItem != null && selectedItem != 'undefined'){
            ...
            if(dms.string.isEmpty(selectedItem.calcDocNo)){
                ...
                $("#parCalcSumAmt").data("kendoExtNumericTextBox").value(selectedItem.parCalcSumAmt);
                $("#parWhDcAmt").data("kendoExtNumericTextBox").value(selectedItem.totAmt);

                $("#parBmPointUseAmt").data("kendoExtNumericTextBox").value(selectedItem.parBmPointUseAmt);
                $("#parPointUseAmt").data("kendoExtNumericTextBox").value(selectedItem.parPointUseAmt);

                    //취소 대상 정산 정보 조회
                    $.ajax({
                         url:"<c:url value='/ser/cal/calculate/selectCalculatePartByKey.do' />"
                        ,data:JSON.stringify({sCalcDocNo:selectedItem.calcDocNo})
                        ,type:'POST'
                        ,dataType:'json'
                        ,contentType:'application/json'
                        ,error:function(jqXHR, status, error) {
            ...
            if(selectedItem.custCd !== 'guest'){
                    $.ajax({
                        ...
                        ,data:JSON.stringify({sCustNo:selectedItem.custCd, sVinNo:''})//$("#vinNo").val()})
                        ,type:'POST'
                        ,dataType:'json'
                        ,contentType:'application/json'
                        ,error:function(jqXHR, status, error) {
                        ...
                        ,success:function(result, textStatus){
                            ...
                            if(dms.string.strNvl(result.membershipNo) != ""){
                                ...
                                $("#gradeNm").val(getDlrMemberGradeCdMap(result.gradeCd));
                        ...
                        $.ajax({
                            ...
                            ,success:function(result){
                                if(result.IsSuccess){
                                    ...
                                    $("#bmLastPointVal").val(result.Data.TotalPoint);

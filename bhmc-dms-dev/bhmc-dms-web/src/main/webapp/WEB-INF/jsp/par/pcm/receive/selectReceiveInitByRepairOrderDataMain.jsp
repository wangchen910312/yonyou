    ...
    console.log('gVatCd:',gVatCd);
           ...
           // 저장 버튼(등록)
            $("#btnSave").kendoButton({
                click:function(e){
                    ...
                    $.ajax({
                         url:"<c:url value='/par/pcm/receive/createReceiveInitByRepairOrderDataReg.do' />"
            ...
            $("#btnCnfm").kendoButton({
                click:function(e){
                    ...
                    $.ajax({
                        url:"<c:url value='/par/pcm/receive/createReceiveInitByRepairOrderDataCnfm.do' />"
                       ,data:kendo.stringify(header)
                       ,type:'POST'
                       ,dataType:'json'
                       ,contentType:'application/json'
            ...
            $("#btnCancel").kendoButton({
                click:function(e){
                    ...
                    $.ajax({
                        url:"<c:url value='/par/pcm/receive/cancelReceiveInitByRepairOrderData.do' />"
                       ,data:kendo.stringify(header)
                       ,type:'POST'
                       ,dataType:'json'
                       ,contentType:'application/json'
      ...
      //부품 팝업 열기 함수.
        function selectItemPopupWindow(){
            ...
            repairOrderInitTargetListPopupWindow = dms.window.popup({
                windowId:"repairOrderInitTargetListPopup"
                , width:860
                , height:550
                , title:"<spring:message code='global.lbl.grForSearch' />"   // 입고대상 조회
                , content:{
                    url:"<c:url value='/par/cmm/selectrepairOrderInitTargetListPopup.do'/>"
                    , data:{
                        "type":""
                        , "autoBind"    :true
                        , "initDataYn":"N"
                        , "selectable"  :"multiple"
                        , "callbackFunc":function(data){
                            repairOrderInitTargetListPopupWindow.close();
                        }
                    }
                }
            });

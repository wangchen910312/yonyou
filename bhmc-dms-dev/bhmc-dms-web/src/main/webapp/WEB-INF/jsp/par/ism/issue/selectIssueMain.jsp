...
<div id="resizableContainer">
    <section class="group">
        ...
        <div class="header_area">
            <div class="btn_right">
                <button class="btn_s btn_add btn_s_min5" id="btnPurcOrd" name="btnPurcOrd"><spring:message code="par.btn.copyPurcOrd" /></button>
                <button class="btn_s btn_add btn_s_min5" id="btnRowAdd" name="btnRowAdd"><spring:message code="par.btn.rowAdd" /></button>
                <button class="btn_s btn_del btn_s_min5" id="btnRowDel" name="btnRowDel"><spring:message code="par.btn.rowDel" /></button>
            </div>
        </div>
        <!-- 그리드 기능 버튼 종료 -->
...
//상태
statusCdList.push({"cmmCd":" ", "cmmCdNm":" ", "useYn":""});
<c:forEach var="obj" items="${statusCdList}" varStatus="status">
    statusCdList.push({"cmmCd":"${obj.cmmCd}", "cmmCdNm":"${obj.cmmCdNm}", "useYn":"${obj.useYn}"});
</c:forEach>
            ...
            if(fieldName === "invcTotAmt"){
                ...
                input.blur(function(){
                    ...
                    if(invcTotAmt > 0 && invcTotAmt != null){
                        ...
                        if(vatCd !== '01'){
                           // 금액
                            invcAmt = Number((invcTotAmt * 100) / (100 + ( gVatCd * 100 ))).toFixed(2);
                            vatAmt = Number((invcAmt * ( gVatCd * 100 )) / 100 ).toFixed(2);
                        }else{
                            // 금액 = 총액
                            invcAmt = invcTotAmt;
                            vatAmt = 0;

...
<c:forEach var="obj" items="${giStrgeCdList}">
    ...
    function setItemList(frGrid, toGrid, rows, avlbStockQtyUseYn){
        ...
        $.each(rows, function(idx, row){
            ...
            rowObj.movingAvgTaxDdctPrc   = frGrid.dataItem(row).movingAvgTaxDdctPrc;
            ...
            /*if(frGrid.dataItem(row).endChk === 'N'){
                ...
                if(popItemObj[frGrid.dataItem(row).itemCd].hasOwnProperty(frGrid.dataItem(row).giStrgeCd)){
                    ...
                    if(avlbStockQtyUseYn){//가용재고 체크없이 모두
                        ...
                        var avlbStockQty = kendo.parseInt(frGrid.dataItem(row).avlbStockQty);
                        if(avlbStockQty > 0){
                            popItemObj[frGrid.dataItem(row).itemCd][frGrid.dataItem(row).giStrgeCd] = frGrid.dataItem(row).itemNm;
                            toGrid.dataSource.add(rowObj);
                        }
    ...
    $(document).ready(function() {
            ...
            click:function(e){
                ...
                if(rows !== null){
                    ...
                    $.each(rows, function(idx, row){
                        ...
                        rowObj.movingAvgPrc   = frGrid.dataItem(row).movingAvgPrc;
                        rowObj.carlineCd      = frGrid.dataItem(row).carlineCd;
                        rowObj.giStrgeCd      = frGrid.dataItem(row).strgeCd;
                ...
                transport:{
                    ...
                    , parameterMap:function(options, operation) {
                        if (operation === "read") {
                            ...
                            params["sEndChk"] = "N";//유효종료일자 체크 안함.
                            params["sTaxDdctGrtePrcOverZeroUseYn"] = "N";//보증가 체크.
        ...
        *************************************************************/
        $("#selectGrid").kendoExtGrid({
           ...
           ,dataSource:[]
            ...
            ,columns:[
                ...
                ,{field:"avlbStockQty",title:"<spring:message code='par.lbl.avlbStockQty' />",attributes:{ "class":"ar"},format:"{0:n2}" ,decimal:2,width:70 }                //가용재고
        ...
        if("${isTablet}" == true || "${isTablet}" == "true"){
            $("#selectGrid").width(910); //khskhs 문제시 삭제
        }
        ...
        $("#shareItemGrid").kendoExtGrid({
           ...
           ,dataSource:{
            ...
            ,columns :
                ...
                ,{field:"avlbStockQty", title:"<spring:message code='par.lbl.avlbStockQty'/>",format:"{0:n2}", attributes:{"class":"ar"}, width:70 }
                ,{field:"salePrcAmt" ,title:"<spring:message code='par.lbl.salePrcAmt' />" ,attributes:{ "class":"ar"} ,format:"{0:n2}" ,width:100 }
                ,{field:"movingAvgPrc"
                    ,title:"<spring:message code='par.lbl.movingAvgAmt' />"
                    ,attributes:{ "class":"ar"}

    ...
    <div class="clfix">
        <section class="left_areaStyle" style="width:550px;">
            <div class="header_area">
                ...
                <div class="btn_right">
                    <dms:access viewId="VIEW-D-13033" hasPermission="${dms:getPermissionMask('READ')}">
            ...
            <div class="header_area">
                <h2 class="title_basic"><spring:message code='cmm.title.mobileMainSetting' /><!-- 모바일메인화면설정 --></h2>
            </div>
            <div class="table_form">
                <c:forEach var="obj" items="${mobMainTpList}">
                <input type="radio" id="mobMainTp${obj.cmmCd}" name="mobMainTp" value="${obj.cmmCd}" />${obj.cmmCdNm}&nbsp;
                </c:forEach>
            </div>
        ...
        <section style="margin-left:560px;">
            <div class="header_area">
                <div class="btn_right">
                    <dms:access viewId="VIEW-D-13034" hasPermission="${dms:getPermissionMask('READ')}">
                        <button type="button" class="btn_m btn_save" id="btnSave"><spring:message code='global.btn.save' /><!-- 저장 --></button>
                    </dms:access>
                    <dms:access viewId="VIEW-D-13035" hasPermission="${dms:getPermissionMask('READ')}">
...
$(document).ready(function() {
    ...
    $("#btnUserSearch").kendoButton({
        click:function(e) {
            ...
            $('input[name="mobMainTp"]').prop("checked", false);
    ...
    $("#btnSave").kendoButton({
        click:function(e) {
            ...
            //모바일메인화면-사용자매핑 저장
            $.ajax({
                url:"<c:url value='/cmm/ath/userPermission/insertOrUpdateMobileMainMapping.do' />"
                ,data:JSON.stringify({
                    "usrId":selectedUsrId
                    ,"mobMainTp":$("input[name=mobMainTp]:checked").val()
                })
                ,type:'POST'
                ,dataType:'json'
                ,contentType:'application/json'
    ...
    $("#userGrid").kendoExtGrid({
        ...
        ,change:function(e){
            ...
            //2.사용자메뉴를 메뉴트리에 체크한다.
            $.ajax({
                url:"<c:url value='/cmm/ath/userPermission/selectMobileMainMapping.do' />"
                ,data:JSON.stringify({"usrId":selectedUsrId})
                ,type:'POST'
                ,dataType:'json'
                ,contentType:'application/json'
                ,error:function(jqXHR, status, error) {
                ...
                ,success:function(data, textStatus, jqXHR) {
                    if(dms.string.isNotEmpty(data.mobMainTp)){
                        $("input[name=mobMainTp][value="+data.mobMainTp+"]").prop("checked", true);
                    }
                }
            });

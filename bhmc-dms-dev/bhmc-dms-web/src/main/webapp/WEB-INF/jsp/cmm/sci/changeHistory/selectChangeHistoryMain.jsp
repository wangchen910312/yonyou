...
<section class="group">
    ...
    <div class="header_area">
        ...
        <div class="btn_right">
	        <dms:access viewId="VIEW-D-10904" hasPermission="${dms:getPermissionMask('READ')}">
...
//시스템
var arrSystemTp = [{"cmmCd":"", "cmmCdNm":"", "useYn":""}];
<c:forEach var="obj" items="${arrSystemTp}">
    arrSystemTp.push({"cmmCd":"${obj.cmmCd}", "cmmCdNm":"${obj.cmmCdNm}", "useYn":"${obj.useYn}"});
</c:forEach>
var systemTpMap = dms.data.arrayToMap(arrSystemTp, function(obj){ return obj.cmmCd; });

//실행유형
var arrWorkTp = [{"cmmCd":"", "cmmCdNm":"", "useYn":""}];
<c:forEach var="obj" items="${arrWorkTp}">
    arrWorkTp.push({"cmmCd":"${obj.cmmCd}", "cmmCdNm":"${obj.cmmCdNm}", "useYn":"${obj.useYn}"});
</c:forEach>
var workTpMap = dms.data.arrayToMap(arrWorkTp, function(obj){ return obj.cmmCd; });

// 상세정보 팝업
var detailPopupWin;
function fnDetailPopup(sChgHisSeq) {

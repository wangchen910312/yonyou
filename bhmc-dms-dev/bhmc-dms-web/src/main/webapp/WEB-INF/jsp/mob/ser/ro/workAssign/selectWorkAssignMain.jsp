                ...
                <table  class="flist01">
                    ...
                    <tbody>
                       ...
                       <%-- <tr>
                            ...
                            <td>
                                <input id="bayNo" type="hidden" disabled="disabled"/>
...
function contentDetail(dataItem) {
    ...
    //$("#qtTestMthCd").val(qtTestCdGrid(dataItem.qtTestMthCd));
    ...
    if(dataItem != null){
        ...
        $("#roDocNo_").val(dataItem.roDocNo);//id가 겹쳐서 _붙였다.
        $("#atqProcTp").val(changeAtqProcTpCd(dataItem.atqProcTp));
        $("#custWaitMthCd").val(changeCustWaitMthCd(dataItem.custWaitMthCd));//고객대기방식
        $("#saNm").val(dataItem.saNm);//배정자
        $("#qtTestCd").val(qtTestCdGrid(dataItem.qtTestCd));
        $("#carWashTp").val(changeCarWashTpCd(dataItem.carWashTp));
        $("#expcDlDt").val(dataItem.expcDlDt);//예정인도시간
        $("#bayNo").val(dataItem.bayNo);
        $("#allocStatCd").val(allocStatCdGrid(dataItem.allocStatCd));//id가 겹쳐서 _붙였다.
    }
    */

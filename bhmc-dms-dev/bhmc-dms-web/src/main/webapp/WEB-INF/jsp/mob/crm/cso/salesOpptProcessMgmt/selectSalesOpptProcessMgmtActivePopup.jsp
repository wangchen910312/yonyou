    ...
    <section id="content">
        ...
        <section id="content_pop_list">
            <div class="content_title">
                ...
                <!-- <span class="title_btn" id="btnCancel"><span>닫기</span></span> -->
            </div>
                ...
                <table  class="flist01">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="f_item01">
                                    <input id="traceDt" type="search" name="traceDt" data-json-obj="true"  />
                                    <span class="date" onclick="calpickerWithTime('traceDt','traceTime')">날짜선택</span>
                                </div>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="f_item01">
                                    <input id="nextTraceDt" type = "search" name="nextTraceDt" data-json-obj="true"  />
                                    <span class="date" onclick="calpickerWithTime('nextTraceDt','nextTraceTime')">날짜선택</span>
                                </div>
                            </td>
                        </tr>
                        <tr>
    ...
    $("#traceGradeCd").kendoExtDropDownList({
        ...
        , select:function(e){
            ...
            if(traceGrdMap[dataItem.cmmCd] != "" && typeof(traceGrdMap[dataItem.cmmCd]) != "undefined" ){
                $("#nextTraceDt").val(date_add(parent.toDt, parseInt(traceGrdMap[dataItem.cmmCd])));
                $("#nextTraceTime").val(time);
            }else{
                //$("#nextTraceDt").val(date_add(parent.toDt, 0));
            }


/*             if(dataItem.cmmCd == "H"){
                $("#nextTraceDt").val(traceGrdMap[dataItem.cmmCd]);
                $("#nextTraceTime").val(time);
            }else if(dataItem.cmmCd == "A"){
                $("#nextTraceDt").val(date_add(parent.toDt, 15));
                $("#nextTraceTime").val(time);
            }else if(dataItem.cmmCd == "B"){
                $("#nextTraceDt").val(date_add(parent.toDt, 30));
                $("#nextTraceTime").val(time);
            }else if(dataItem.cmmCd == "C"){
                $("#nextTraceDt").val(date_add(parent.toDt, 90));
                $("#nextTraceTime").val(time);
            }else if(dataItem.cmmCd == "D"){
                $("#nextTraceDt").val(date_add(parent.toDt, 180));
                $("#nextTraceTime").val(time);
            }else{
                $("#nextTraceDt").val("");
                $("#nextTraceTime").val("");
    ...
    if (options.autoBind) {
        ...
        if(traceGrdMap[options.traceGradeCd] != "" && typeof(traceGrdMap[options.traceGradeCd]) != "undefined" ){
            $("#nextTraceDt").val(date_add(parent.toDt, parseInt(traceGrdMap[options.traceGradeCd])));
            $("#nextTraceTime").val(time);
        }else{
            //$("#nextTraceDt").val(date_add(parent.toDt, 0));
        }
...
$("#btnCtlSaveTrace").kendoButton({
    click:function(e){
        ...
        if ( !dms.string.isEmpty(nextTraceMthCd) ){
            if(dms.string.isEmpty(nextTraceDt)||dms.string.isEmpty(nextTraceTime)){
        ...
        /* if ( dms.string.isNotEmpty(nextTraceDt)){
            if (dms.string.isEmpty(nextTraceMthCd)) {
                mob.notification.warning("<spring:message code='crm.lbl.nextTraceMthCd' var='returnMsg' /><spring:message code='global.info.emptyParamInfo' arguments='${returnMsg}' />");
                return false;
            }
        } */
        ...
        if(dms.string.isEmpty(sTraceDt)){
            ...
            salesOpptTraceParam["traceDt"] = chgDateTimeStamphhMM($("#traceDt").val() + " " + $("#traceTime").val());
        }
        ...
        if(dms.string.isEmpty(sNextTraceDt)){
            ...
            salesOpptTraceParam["nextTraceDt"] = chgDateTimeStamphhMM($("#nextTraceDt").val() + " " + $("#nextTraceTime").val());
        }
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                $("#traceDt").data("kendoDateTimePicker").value("");
                $("#traceCont").val("");
                ...
                $("#nextTraceDt").data("kendoDateTimePicker").value(""); */

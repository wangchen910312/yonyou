      ...
      <div id="resizableContainer">
            <section class="group">
                <div class="header_area">
                    ...
                    <div class="btn_right">
                    <dms:access viewId="VIEW-D-11239" hasPermission="${dms:getPermissionMask('READ')}">
                        <button type="button" class="btn_m" id="btnCopyVehic"><spring:message code="global.btn.copy" /></button> <!-- 복사 -->
                    </dms:access>
                    <dms:access viewId="VIEW-D-11240" hasPermission="${dms:getPermissionMask('READ')}">
                        <button type="button" class="btn_m btn_search" id="btnSearch"><spring:message code="global.btn.search" /></button>  <!-- 조회 -->
                    </dms:access>
                    <dms:access viewId="VIEW-D-11241" hasPermission="${dms:getPermissionMask('READ')}">
                        <button type="button" class="btn_m btn_new" id="btnAddVehic" ><spring:message code="global.btn.new" /></button> <!-- 신규  -->
                    </dms:access>
                    <dms:access viewId="VIEW-D-11242" hasPermission="${dms:getPermissionMask('READ')}">
                        <button type="button" class="btn_m btn_modify" id="btnViewDetail" ><spring:message code='global.btn.update' /><!-- 수정 --></button>
                    </dms:access>
                    <dms:access viewId="VIEW-D-11243" hasPermission="${dms:getPermissionMask('READ')}">
                        <button type="button" class="btn_m btn_print" id="btnPrintVehic" ><spring:message code="global.btn.print" /></button> <!-- 인쇄 -->
                    </dms:access>
                    <dms:access viewId="VIEW-D-11244" hasPermission="${dms:getPermissionMask('READ')}">
...
<script type="text/javascript" charset="UTF-8">
    ...
    function resetDefualtForm(){
        ...
        $("#targCustCnt").data("kendoExtNumericTextBox").value(0);
        $("#makCont").val("");
        $("#regDt").val("");
        $("#updtDt").val("");
        $("#regUsrId").val("");
    ...
    function onSelectMakTpCdSet(value) {
        ...
        if(value == ''){
            ...
            //市场活动里新建时，选中售后之后，二级菜单需要把厂家市场活动类型的值，画面上不显示， C，R，K，T，V,共同编码需要保留  贾明 2019-1-21 start
            if('03' == value){
               var arr = new Array();
            ...
            //市场活动里新建时，选中售后之后，二级菜单需要把厂家市场活动类型的值，画面上不显示， C，R，K，T，V,共同编码需要保留  贾明 2019-1-21 end
        ...
        $("#tabstrip").kendoExtTabStrip({
            ...
            , select:function(e) {
                ...
                if ( dms.string.isNotEmpty(makCd) ) {
                    ...
                    } else if (selectTabId == "dlrLbrTab") {    // 딜러 공임정보(정비용) 그리드 조회
                        $("#dlrLbrGrid").data("kendoExtGrid").dataSource.page(1);
                    }  else if (selectTabId == "dlrItemTab") {  // 딜러 부품정보(정비용) 그리드 조회
                        onSelectLbrCdSet(dlrCd, makCd);
...
//     $("#sEndDt").on('change', {from:$("#sStartDt"), to:$("#sEndDt")}, fnChkSearchDate);     // 시작일은 종료일보다 클수가 없다.
       ...
       click:function(e){
           ...
           if(selectedItem != null){
               ...
               if ( selectedItem.hostCd !== "02" ) {
                   ...
                   dms.notification.warning("<spring:message code='crm.info.noMakeCorpCopy'/>");
                   return false;
               };
    ...
    $("#btnCtlCamMgmtExcelExp").kendoButton({
        click:function(e) {
            ...
            $.ajax({
                url:"<c:url value='/crm/mcm/marketingCampaign/selectMarketingCampaignsExcelExportCnt.do' />",
    ...
    $("#btnSaveVehic").kendoButton({
       click:function(e){
           ...
           json.statCd      = $("#statCd").data("kendoExtDropDownList").value();
...
//                            selectRow = gridBudget.tbody.find(">tr:not(.k-grouping-row)").eq(i);
//                            gridBudget.select(selectRow);
//                            var td = selectRow.find("td:eq(5)");
//                            console.log(td);
//                            $(td).focus();
//                            td.focus();
                           return;
    ...
    $("#btnBudgetExcelExport").kendoButton({
        click:function(e) {
            ...
            $.ajax({
                url:"<c:url value='/crm/mcm/marketingCampaign/selectBudgetsExcelExportCnt.do' />",
                data:JSON.stringify(params),
                type:'POST',
                dataType:'json',
                contentType:'application/json',
...
//                     정상적으로 반영 되었습니다.
    ...
    $("#btnLtsModelExcelImport").kendoButton({
        click:function(e) {
            ltsUploadPopupWin = dms.window.popup({
                title:"<spring:message code='crm.btn.excelLtsModelUpload' />"
                ,width:800
                ,height : 400
                ,modal : true
                ,windowId:"ltsUploadPopupWin"
                ,content:{
                    url:"<c:url value='/crm/mcm/marketingCampaign/selectLTSUploadPopup.do?dlrCd="+$("#dlrCd").val()+"&makCd="+$("#makCd").val()+"'/>"
                    ,data:{
                        "uploadFlag" : "LTS"
                       ,"callbackFunc":function(){
                            $('#vinGrid').data('kendoExtGrid').dataSource.read();
                            ltsUploadPopupWin.close();
                        }
                    }
                }
            });
    ...
    $("#mcdGrid").kendoExtGrid({
        ...
        ,dataBound:function(e) {
            ...
            if ( dataItems.length > 0 ) {
                $("#btnCtlCamMgmtExcelExp").data("kendoButton").enable(true);
            } else {
                $("#btnCtlCamMgmtExcelExp").data("kendoButton").enable(false);
            };
        }
        ,columns:[
...
//              console.log("selectedItem.custExtrTermNo!!");
//              console.log(selectedItem.custExtrTermNo);
             ...
             if ( selectedItem != null &&
                  selectedItem.custExtrTermNo != 'EXCELUPLOAD' && selectedItem.custExtrTermNo != 'DCS_IF_VIN_CUSTNO' &&
                  selectedItem.custExtrTermNo != 'DCS_IF_MD_CUSTNO' && selectedItem.custExtrTermNo != 'DCSIFUPLOAD' ) {
         ...
         , columns:[
             ...
             ,{field:"custExtrTitleNm", title:"<spring:message code='crm.lbl.custExtrTermNo' />", width:200 ,attributes:{"class":"al"}
                 ,template:function(dataItem){
                     ...
                     var custExtrTermNo = dataItem.custExtrTermNo;
                     var spanObj = dataItem.custExtrTitleNm ;
                     // DCS에서 or 고객업로드로 추출실행 시 링크 해제
                     if( custExtrTermNo != 'EXCELUPLOAD'  && custExtrTermNo != 'DCS_IF_VIN_CUSTNO' &&
                         custExtrTermNo != 'DCS_IF_MD_CUSTNO' && custExtrTermNo != 'DCSIFUPLOAD' ) {
                         spanObj = "<a href='#' class='linkCustExtrTermNo'>"+dataItem.custExtrTitleNm+"</a>";
                     }
...
//              ,footerTemplate:"#=kendo.toString(sum, 'n0')#"
           ...
           ,{field:"imsSuptAmt", title:"<spring:message code='global.lbl.imsSuptAmt' />", width:150, attributes:{"class":"ar"},  format:"{0:n2}"
//              ,footerTemplate:"#=kendo.toString(sum, 'n0')#"
             ...
             ,schema:{
                 model:{
                     ...
                     ,fields:{
                         ...
                         ,signUsrNm:{type:"string", validation:{required:true},editable:false}
    ...
    $("#resultGoalGrid").kendoExtGrid({
        ...
        ,columns:[
           ...
           ,{_field:"achievementRate", title:"<spring:message code='crm.lbl.achievementRate' />", width:60, attributes:{"class":"ar"}
    ...
    $("#resultCorpGrid").kendoExtGrid({
        ...
        ,columns:[
           ...
           ,{field:"stsfIvstScoreVal", title:"<spring:message code='global.lbl.value' />", width:80, attributes:{"class":"ar"}
               ,template:function(dataItem){
                   var spanObj = "<a href='#' class='corpEmpNm'>"+kendo.format("{0:n2}", dataItem.stsfIvstScoreVal)+"</a>";
                   return spanObj;
               }
    ...
    $("#dlrLbrGrid").kendoExtGrid({
        ...
        ,columns:[
            ...
            ,{field:"lbrHm", title:"<spring:message code='ser.lbl.rpirDt' />", width:80
                ,attributes:{"class":"ar"}
                ,format:"{0:n2}"
                ,editor:function(container, options){
                    $('<input required name="lbrHm" data-name="<spring:message code="ser.lbl.rpirDt" />" data-bind="value:' + options.field + '"  />')
                    .appendTo(container)
                    .kendoExtNumericTextBox({
                         min:0
                        ,max:1000

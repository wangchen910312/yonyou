...
<div class="fsearch_area">
    ...
    <div class="table_form" role="search" data-btnid="btnReportSearch">
        <table>
            ...
            <tbody>
                <tr>
                    <th scope="row"><spring:message code='crm.lbl.cupnUseDt' /><!-- 쿠폰사용일  --></th>
                    <td>
                        <div class="form_float">
                            <div class="date_left">
                                <input id="sUsedStartDt" class="form_datepicker"/>
                                <span class="txt_from">~</span>
                            </div>
                            <div class="date_right">
                                <input id="sUsedEndDt" class="form_datepicker"/>
                            </div>
                        </div>
                    </td>
...
<script type="text/javascript">
    ...
    function reportCall(){
        ...
        reportParam += "&sUsedStartDt="+$("#sUsedStartDt").val();
        reportParam += "&sUsedEndDt="+$("#sUsedEndDt").val();
        $("#iframeReport").attr("src", "<c:url value='/ReportServer?reportlet=crm/BMReport/selectBMCouponReport.cpt' />&op=view"+reportParam);
    };

    $(document).ready(function() {
        $("#btnReportSearch").kendoButton({
            click:function(e) {
                if(crmJs.isDateValidPeriod($("#sUsedStartDt").data("kendoExtMaskedDatePicker").value()
                        ,$("#sUsedEndDt").data("kendoExtMaskedDatePicker").value(),'<spring:message code="global.err.checkDateValue"/>')){
                    ...
                    $("#sUsedEndDt").data("kendoExtMaskedDatePicker").value(new Date(''));
                    $("#sUsedEndDt").focus();
                }
            }
        });
        ...
        $("#sUsedStartDt").kendoExtMaskedDatePicker({
            format:"<dms:configValue code='dateFormat' />"
            ,value:"<c:out value='${sStartDt}' />"
            ,dateValidMessage:"<spring:message code='global.lbl.date' var='dateMsg' /><spring:message code='global.err.invalid' arguments='${dateMsg}' />"
        });
        $("#sUsedEndDt").kendoExtMaskedDatePicker({
            format:"<dms:configValue code='dateFormat' />"

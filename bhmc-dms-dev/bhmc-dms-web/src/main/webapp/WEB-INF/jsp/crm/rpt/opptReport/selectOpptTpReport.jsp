        ...
        <table>
            ...
            <tbody>
                <tr>
                    ...
                    <th scope="row"><spring:message code='global.lbl.newRegDt' /></th><!--등록일 -->
                    <td>
...
<div class="table_info table_info_v1">
    <iframe id="iframeReport" name="iframeReport" src="" frameborder="0" scrolling="no" width="100%" height="590" class="report"></iframe>
</div>
...
holdTpList.push({"cmmCd":"${obj.cmmCd}", "cmmCdNm":"${obj.cmmCdNm}", "useYn":"${obj.useYn}"});
    ...
    function reportCall(){
        ...
        $("#iframeReport").attr("src", "<c:url value='/ReportServer?reportlet=crm/opptReport/selectOpptTpReport.cpt' />&op=view"+reportParam);
    };
        ...
        $("#btnReportSearch").kendoButton({
            click:function(e) {
                if(crmJs.isDateValidPeriod($("#sRegStartDt").data("kendoExtMaskedDatePicker").value()
                    ...
                    if(crmJs.isDateValidPeriod($("#sActStartDt").data("kendoExtMaskedDatePicker").value()
                            ,$("#sActEndDt").data("kendoExtMaskedDatePicker").value(),'<spring:message code="global.err.checkDateValue"/>')){
                        reportCall();
                    }else{
                        $("#sActEndDt").data("kendoExtMaskedDatePicker").value(new Date(''));
                        $("#sActEndDt").focus();
                    }
                }else{

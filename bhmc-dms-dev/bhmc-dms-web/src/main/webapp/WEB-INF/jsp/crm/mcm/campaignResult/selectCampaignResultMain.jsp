        ...
        <div id="tabstrip" class="tab_area">
            ...
            <div id="rsltInputTabDiv" class="mt0">
                ...
                <div class="header_area">
                    ...
                    <div class="btn_right">
                    <dms:access viewId="VIEW-D-11283" hasPermission="${dms:getPermissionMask('READ')}">
                ...
                <div class="table_form" >
                    <table>
                        ...
                        <tbody>
                            <tr>
                                <th scope="row"><spring:message code="crm.lbl.actTotAmt"/></th><!-- 활동 총 비용 -->
                                <td>
                ...
                <div class="header_area">
                    ...
                    <div class="btn_right">
					<dms:access viewId="VIEW-D-11284" hasPermission="${dms:getPermissionMask('READ')}">
            ...
            <div id="cropTabDiv" class="mt0">
                ...
                <div class="header_area">
                    ...
                    <div class="btn_right">
                    <dms:access viewId="VIEW-D-11302" hasPermission="${dms:getPermissionMask('READ')}">
                        <button class="btn_s" id="btnCropSearch"><spring:message code="crm.btn.survey" /></button><!-- 설문 -->
					</dms:access>
					<dms:access viewId="VIEW-D-11303" hasPermission="${dms:getPermissionMask('READ')}">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <input type="hidden" id="stsfIvstRsltSeq" name="stsfIvstRsltSeq" >
                                </td>
...
***************************************************************************-->
                          <section class="right_areaStyle">
                              <div class="header_area">
                                  <div class="btn_right">
								  <dms:access viewId="VIEW-D-11285" hasPermission="${dms:getPermissionMask('READ')}">
...
<%--                                       <button class="btn_s" id="btnSaveFile" ><spring:message code="global.btn.save" /><!-- 파일저장 --></button> --%>
                                    <dms:access viewId="VIEW-D-11286" hasPermission="${dms:getPermissionMask('READ')}">
                                      <button type="button" class="btn_s" id="btnPrsnContSave" ><spring:message code="global.btn.save" /></button> <!-- 저장 -->
                                    </dms:access>
                                    <dms:access viewId="VIEW-D-11287" hasPermission="${dms:getPermissionMask('READ')}">
                                      <button class="btn_s" id="fileDelete"><spring:message code="global.btn.fileDelete" /><!-- 파일삭제 --></button>
                                    </dms:access>
                                    <dms:access viewId="VIEW-D-11288" hasPermission="${dms:getPermissionMask('READ')}">
                ...
                if (selectTabId == "rsltInputTab") {
                    ...
                    $.ajax({
                        ...
                        ,success:function(jqXHR, textStatus) {
                            ...
                            var stsfIvstRsltSeq = jqXHR.stsfIvstRsltSeq;

                            var sSeqDDL = $("#sSeq").data("kendoExtDropDownList");

                            // 결과가 있는경우 결과까지 조회후 읽기전용
                            if ( subyTmplSeq > 0 ) {

                                $("#stsfIvstRsltSeq").val(stsfIvstRsltSeq);
...
*************************************************************/
    ...
    $("#btnCtlRltMgmtExcelExp").kendoButton({
        click:function(e) {
            ...
            // 캠페인결과 목록 엑셀Export 수량 데이터를 조회한다.
            $.ajax({
                url:"<c:url value='/crm/mcm/campaignResult/selectCampaignResultMgmtsExcelExportCnt.do' />",
                ...
                success:function(resultCnt) {
                    ...
                    if ( $("#mcdGrid").data("kendoExtGrid").dataSource._total > excelMaxExportCnt ) {
    ...
    $("#btnDefaultTargSum").kendoButton({
        click:function(e) {
            ...
            // 기본목표 결과계산 confirm
            dms.window.confirm({
                message:"<spring:message code='crm.info.defaultTargSumConfirm'/>"       // 현재 캠페인의 진행상태가 [심사완료]인  전체 기본목표값이 계산됩니다. 기본목표값 계산을 수행하시겠습니까? -> 기본 목표값 계산을 수행하시겠습니까?(20170607 유정 수정)
                    ...
                    ,callback:function(result){
                        ...
                        if ( result ) {
                            ...
                            $.ajax({
                                ...
                                ,type:"POST"
                                ,contentType:"application/json"
                                ,data:JSON.stringify({"insertList":params, "satisFactionMobileResultVO":satisFactionMobileResultVO, "makCd":$("#makCd").val()})
                                ,error:function(jqXHR,status,error) {
                                    dms.notification.error(jqXHR.responseJSON.errors);
                                }
                                ,success:function(data) {
                                    dms.notification.success("<spring:message code='global.info.success'/>");
                                    $("#btnCropSave").data("kendoButton").enable(false);
                                    $("#survey_content01 > ul").empty();
                                    //설문에 응해주셔서 감사합니다.
    ...
    $("#btnCropSearch").kendoButton({
        click:function(e){
            ...
            if(selectedItem!=null) {
                ...
                $("#survey_content01 > ul").empty();
                var sSeq = $("#sSeq").val();
                var sStsfIvstRsltSeq = $("#stsfIvstRsltSeq").val();
                var sMakCd = $("#makCd").val();
                var sEmpId = $("#sEmpId").val();
                ...
                var params = {
                     "sSeq":sSeq
                    ,"sStsfIvstRsltSeq":sStsfIvstRsltSeq
                    ,"sMakCd":sMakCd
                    ,"sEmpId":sEmpId
                };
                ...
                $.ajax({
                    ...
                    $("#subyTmplSeq").val(result["subyTmplSeq"]);       // ???
                    ...
                    $("#btnCropSave").data("kendoButton").enable(true);
     ...
     $("#mcdGrid").kendoExtGrid({
         ...
         ,dataBound:function(e) {
             ...
             if ( dataItems.length > 0 ) {
                 $("#btnCtlRltMgmtExcelExp").data("kendoButton").enable(true);
             } else {
                 $("#btnCtlRltMgmtExcelExp").data("kendoButton").enable(false);
             };
         }
         ,columns:[
            ...
            ,{field:"visitCnt", title:"<spring:message code='crm.lbl.visitCnt' />", width:100, attributes:{"class":"ar"}, format:"{0:##,###}", sortable:true}    // 내방 수량
            ,{field:"salesOpptCnt", title:"<spring:message code='crm.lbl.salesOpptCnt' />", width:100, attributes:{"class":"ar"}, format:"{0:##,###}", sortable:true}    // 영업기회 수량
            ,{field:"venCnt", title:"<spring:message code='crm.lbl.venCnt' />", width:100, attributes:{"class":"ar"}, format:"{0:##,###}", sortable:true}    // 거래 수량
            ,{field:"venAmt", title:"<spring:message code='crm.lbl.venAmt' />", width:100, attributes:{"class":"ar"}, format:"{0:n2}", sortable:true}    // 거래 금액
            ,{field:"visitResvCnt", title:"<spring:message code='crm.lbl.visitResvCnt' />", width:100, attributes:{"class":"ar"}, format:"{0:##,###}", sortable:true}    // 내방예약 수량
    ...
    //그리드 설정(목표)
     $("#goalResultGrid").kendoExtGrid({
         ...
         ,columns:[
            ...
            ,{field:"targCntVal", title:"<spring:message code='global.lbl.targCntVal' />", width:100, attributes:{"class":"ar"}, format:"{0:##,###}"}   // 목표수치값
            ,{field:"targRsltVal", title:"<spring:message code='global.lbl.targRsltVal' />", width:100, attributes:{"class":"ar"}, format:"{0:##,###}"}   // 목표결과

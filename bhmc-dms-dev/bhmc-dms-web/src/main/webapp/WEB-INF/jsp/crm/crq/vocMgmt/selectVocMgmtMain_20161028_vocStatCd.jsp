...
<%@taglib prefix="dms" uri="/WEB-INF/tlds/taglibs-dms"%>
    ...
    <section class="group">
        ...
        <!--  ************************** 고객F/U중 입력 팝업 **************************** -->
        <section id="vocStat04Popup" class="pop_wrap">
            <div class="header_area">
                <div class="btn_right">
                    <button type="button" class="btn_s" id="btnVocStat04Popup"><spring:message code="crm.btn.proce" /><!-- <!-- 확인 --></button>
                    <button type="button" class="btn_s" id="btnVocStat95Popup"><spring:message code="crm.btn.procReturn" /><!-- <!-- 처리로반송 --></button>
                    <button type="button" class="btn_s" id="btnCancel04Popup"><spring:message code="global.btn.cancel" /><!-- 취소 --></button>
                </div>
            </div>
            <div class="table_form">
                <table>
        ...
        <h2 class="title_basic"><spring:message code="global.title.vocReqAccept" /></h2><!-- VOC 요청접수 -->
            ...
            <dms:access viewId="VIEW-D-10278" hasPermission="${dms:getPermissionMask('READ')}">
                ...
                <button type="button" class="btn_s" name="btnVocStat" id="btnVocStat04"><spring:message code="crm.btn.custFU" /></button><!-- 고객F/U중 - 활성화 : 접수 / 처리완료 설문대기 -->
...
//                 console.log("*고객내방대기");
                $("#btnVocStat04").data("kendoButton").enable(true);    //고객F/U
...
//             console.log("96 : 처리완료설문대기 -> 고개F/U로 반송");
        ...
        } else if ( vocStatCd === "04" ) {
            //console.log("04 : 고객내방대기 -> 고객F/U중 ( 클레임 )");
...
//             console.log("95 : 고객내방대기 -> 고객F/U중 ( 클레임 ) -> 처리로 반송(97)");
        ...
        $.ajax({
            ...
            ,success:function(data) {
                ...
                if ( vocStatCd === "02" || vocStatCd === "03" || vocStatCd === "04" || vocStatCd === "05" || vocStatCd === "99" || vocStatCd === "08" || vocStatCd === "07" || vocStatCd === "06" || vocStatCd === "97" || vocStatCd === "96" ) {
...
//         $("#cmplStsfNm02").val("");
    ...
    function btnDisabled(){
        ...
        $("#btnVocStat04").data("kendoButton").enable(false);
        ...
        $("#btnVocStat04Popup").data("kendoButton").enable(false);
        $("#btnVocStat95Popup").data("kendoButton").enable(false);
        $("#btnCancel04Popup").data("kendoButton").enable(false);
...
//                 $("#dstbNo").attr("disabled", false);
    ...
    $("#btnVocStat04").kendoButton({
        click:function(e) {
            $("#btnVocStat04Popup").data("kendoButton").enable(true);
            $("#btnVocStat95Popup").data("kendoButton").enable(true);
            $("#btnCancel04Popup").data("kendoButton").enable(true);

            var win = $("#vocStat04Popup").data("kendoWindow");
            win.center();
            win.open();
        }
        ,enable:false
    });
    // 고객F/U중 팝업 -> 확인 버튼 : 고객내방대기 (08) -> 고객F/U중(04)
    $("#btnVocStat04Popup").kendoButton({
        click:function(e) {
            vocStatUpdt('04')
        }
        ,enable:false
    ...
    // 고객F/U중 팝업 팝업 -> 취소 버튼
    $("#btnCancel04Popup").kendoButton({
        click:function(e) {
            $("#vocStat04Popup").data("kendoWindow").close();
        }
        ,enable:false
    });
    /** 고객F/U중 버튼(클레임) : 고객내방대기 (08) -> 고객F/U중 팝업 *****************************************/
    ...
    // 설문완료CLOSE 팝업 -> 고객F/U로 반송 버튼 : 처리완료설문대기(05) -> 고객F/U(04)
           ...
           ,change:function(e){
               ...
               if( dms.string.isNotEmpty(selectedItem) ){
                   ...
                   $.ajax({
                       ...
                       btnActive(selectedItem.vocStatCd, selectedItem.vocTpCd);
                       $("#bfVocStatCd").val(selectedItem.vocStatCd);
                       // VOC 유형은 수정불가
                       $("#vocTpCd").data("kendoExtDropDownList").value(selectedItem.vocTpCd);
                       $("#vocTpCd").data("kendoExtDropDownList").enable(false);

//                        console.log("selectedItem.vocTpCd ::: ",selectedItem.vocTpCd)

                       /*
                       if ( selectedItem.vocTpCd === "02" ) {
                           $("#btnVocStat08").removeClass("hidden");
                           $("#btnVocStat04").removeClass("hidden");
                       } else {
                           $("#btnVocStat08").addClass("hidden");
                           $("#btnVocStat04").addClass("hidden");
                       };
                       */
                       ...
                       $("#mthAchvHm").data("kendoExtMaskedDateTimePicker").value(selectedItem.mthAchvHm);
                       ...
                       $("#sendCd").data("kendoExtDropDownList").value(selectedItem.sendCd);
                       ...
                       $("#cmplStsfNm01").data("kendoExtDropDownList").value(selectedItem.cmplStsfNm);
                       $("#cmplStsfNm02").data("kendoExtDropDownList").value(selectedItem.cmplStsfNm);
...
//                        $("#cmplStsfNm02").val(selectedItem.cmplStsfNm);
                       ...
                       $("#salesActiveSrcCd").data("kendoExtDropDownList").value(selectedItem.salesActiveSrcCd);
                       $("#relSalesActiveNo").val(selectedItem.relSalesActiveNo);


                       $("#parOrdYn").data("kendoExtDropDownList").value(selectedItem.parOrdYn);
                       $("#parOrdDt").data("kendoExtMaskedDateTimePicker").value(selectedItem.parOrdDt);
                       $("#parArrvDt").data("kendoExtMaskedDateTimePicker").value(selectedItem.parArrvDt);
                       ...
                       if ( dms.string.isNotEmpty(selectedItem.bhmcVocNo) && selectedItem.bhmcYn === "Y" ) {
                           ...
                           console.log("제조사 VOC ");
                           // 상단에서 버튼제어후 BHMC 인경우 다시 제어
                            btnDisabled();

                           if ( selectedItem.vocStatCd === "03" || selectedItem.vocStatCd === "05" ) {
                               ...
                               console.log()
                                ...
                                $("#btnVocStat04").data("kendoButton").enable(true);        //고객F/U
                                $("#btnVocStat04Popup").data("kendoButton").enable(true);   //고객F/U POPUP
                                $("#btnVocStat95Popup").data("kendoButton").enable(true);   //고객F/U POPUP
                                $("#btnCancel04Popup").data("kendoButton").enable(true);    //고객F/U POPUP
                           }


                           $("#realPprocNm").val(selectedItem.realPprocId);
                           $("#pconRelCd").parent().find(".k-input").html(selectedItem.pconTpNm);
                           $("#sungCd").parent().find(".k-input").html(selectedItem.sungNm);
                           $("#cityCd").parent().find(".k-input").html(selectedItem.cityNm);
                           $("#carlineCd").parent().find(".k-input").html(selectedItem.carlineNm);
                           $("#modelCd").parent().find(".k-input").html(selectedItem.modelNm);
                           $("#ocnCd").parent().find(".k-input").html(selectedItem.ocnNm);
                           $("#pacptId").parent().find(".k-input").html(selectedItem.pacptId);
                           $("#cmplStsfNm01").parent().find(".k-input").html(selectedItem.cmplStsfNm);

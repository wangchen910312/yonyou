...
<section class="group">
    <div class="header_area">
        <h1 class="title_basic"><spring:message code='global.title.deliveryAsk' /></h1> <!-- 출고요청관리 -->
        <div class="btn_right">
            <button id="btnAskDelivery" class="btn_m"><spring:message code='global.lbl.releaseReq' /></button>  <!-- 출고요청 -->
            <button id="btnCancelDelivery" class="btn_m"><spring:message code='global.lbl.releaseReqCancel' /></button>    <!-- 출고요청취소 -->
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <div class="form_float">
                            <div class="date_left">
                                <input id="sAllocStartDt" class="form_datepicker">
                                <span class="txt_from">~</span>
                            </div>
                            <div class="date_right">
                                <input id="sAllocEndDt" class="form_datepicker">
                            </div>
                        </div>
                    </td>
                </tr>
...
<c:forEach var="obj" items="${contractStatCdDS}">
    <c:if test="${obj.cmmCd eq '30' || obj.cmmCd eq '40'}">
...
$(document).ready(function() {
    ...
    $("#btnSearch").kendoButton({ // 조회
        ...
        click:function(e){
            ...
            // 출고배정일
            if( ($("#sAllocStartDt").data("kendoExtMaskedDatePicker").value() != null && $("#sAllocEndDt").data("kendoExtMaskedDatePicker").value() == null)
                ||($("#sAllocEndDt").data("kendoExtMaskedDatePicker").value() != null && $("#sAllocStartDt").data("kendoExtMaskedDatePicker").value() == null)){
                //적용일을 모두 입력해 주세요
    ...
    $("#btnAskDelivery").kendoButton({ // 출고요청
        enable:true,
        click:function(e){
            bf_btnClick("btnAskDelivery");
        }
    });

    $("#btnCancelDelivery").kendoButton({ // 출고요청취소
        enable:true,
        click:function(e){
    ...
    // 배정 시작일
    $("#sAllocStartDt").kendoExtMaskedDatePicker({
    ...
    // 배정 종료일
    $("#sAllocEndDt").kendoExtMaskedDatePicker({
        format:"<dms:configValue code='dateFormat' />"
        ,dateValidMessage:"<spring:message code='global.lbl.date' var='dateMsg' /><spring:message code='global.err.invalid' arguments='${dateMsg}' />"
        ,value:toDay
    });
  ...
  // 배정 시작일, 종료일
  $("#sAllocStartDt").on('change', {from:$("#sAllocStartDt"), to:$("#sAllocEndDt")}, fnChkSearchDate);
  $("#sAllocEndDt").on('change',   {from:$("#sAllocStartDt"), to:$("#sAllocEndDt")}, fnChkSearchDate);
  // 출고요청 시작일, 종료일
  ...
  $("#grid").kendoExtGrid({
      dataSource:{
          transport:{
              read:{
                  url:"<c:url value='/sal/ctm/deliveryAsk/selectDeliveryAsks.do' />"
              },
              parameterMap:function(options, operation) {
                  if (operation === "read") {
                      ...
                      // 배정 시작일
                      params["sAllocStartDt"] = $("#sAllocStartDt").data("kendoExtMaskedDatePicker").value();
                      // 배정 종료일
                      params["sAllocEndDt"] = $("#sAllocEndDt").data("kendoExtMaskedDatePicker").value();
                      // 계약번호
     ...
     ,columns:[
        ...
        ,{field:"allocDt", title:"<spring:message code='global.lbl.allocDt' />", width:100, attributes:{"class":"ac"}, format:"{0:"+vDtyyyyMMdd+"}"} // 배정일
    ...
    bf_btnClick = function(mg){
        ...
        // 출고요청 팝업 호출
        if(mg == "btnAskDelivery"){

            if(item.contractStatCd != "30"){
                // 해당 내역은 배정 상태가 아닙니다.
                dms.notification.info("<spring:message code='global.lbl.assign' var='globalLblAssign' /><spring:message code='global.info.stateNotMsg' arguments='${globalLblAssign}' />");
                return false;
            }
            bf_btnAskDelivery(item);

        }   // 출고요청 취소
        else if(mg == "btnCancelDelivery"){
            ...
            $.ajax({
                url:"<c:url value='/sal/ctm/deliveryAsk/confirmAskDeliveryCancel.do' />",
    ...
    // 출고요청 팝업
    bf_btnAskDelivery = function(item){
        ...
        popupWindow = dms.window.popup({
            ...
            ,width :800
            ,content:{
                url:"<c:url value='/sal/ctm/deliveryAsk/selectDeliveryAskPopup.do'/>"
                ,data:{
                    "type":"custom1"
                    ,"autoBind":false
                    ...
                    ,"contractTp":item.contractTp
                    ,"custNm"    :item.custNm
                    ,"carlineNm" :item.carlineNm
                    ,"modelNm"   :item.modelNm
                    ,"ocnNm"     :item.ocnNm
                    ,"extColorNm":item.extColorNm
                    ,"intColorNm":item.intColorNm
                    ,"vinNo"     :item.vinNo
                    ,"contractDt":item.contractDt
                    ,"allocDt"   :item.allocDt
                    ,"dlReqDt"   :item.dlReqDt
    ...
    if("${contNo}" != ""){
        ...
        $("#sAllocStartDt").data("kendoExtMaskedDatePicker").value('');  // 계약일
        $("#sAllocEndDt").data("kendoExtMaskedDatePicker").value('');    // 계약일
        $("#sGiReqStartDt").data("kendoExtMaskedDatePicker").value('');     // 인도요청일
        $("#sGiReqEndDt").data("kendoExtMaskedDatePicker").value('');       // 인도요청일

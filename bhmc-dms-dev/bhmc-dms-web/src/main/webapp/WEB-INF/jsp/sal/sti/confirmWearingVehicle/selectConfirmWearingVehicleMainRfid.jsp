...
<div id="resizableContainer">
    <section class="group">
        <div class="header_area">
            ...
            <div class="btn_right">
                <dms:access viewId="VIEW-D-13147" hasPermission="${dms:getPermissionMask('READ')}">
                    <button class="btn_m" id="btnReceiveCnfrm"><spring:message code="par.btn.receiveCnfm" /></button> <!-- 입고확정 -->
                </dms:access>
                <dms:access viewId="VIEW-D-13148" hasPermission="${dms:getPermissionMask('READ')}">
                    <button class="btn_m" id="btnReceiveCncl"><spring:message code="global.btn.receiveCncl" /></button> <!-- 입고취소 -->
                </dms:access>
                <dms:access viewId="VIEW-D-13149" hasPermission="${dms:getPermissionMask('READ')}">
        ...
        <div class="header_area">
            <div class="btn_right">
            <dms:access viewId="VIEW-D-13146" hasPermission="${dms:getPermissionMask('READ')}">
     ...
     function fnChkVehiclePopup(){
         ...
         if( rows.length > 1){
             rows.each(function(index, row) {
                 ...
                 /**
                    RFID 체크 로직   :
                    1. VIN 상태 코드가 'A' 일경우만 입고  가능
                    2. SAL220 에 오더 타입이 동일할경우 스캔 정보 없이도 입고 가능 . - 2017.09.12 추가
                 **/
                 if( gridData.skipYn != "Y" && gridData.statCd != "A" ){
    	...
    	$("#grid").kendoExtGrid({
            ...
            ,change:function(e) {
                ...
                $.each(rows, function(idx, row){
                      if( (dms.string.trim(that.dataItem(row).contractNo) != "NONE" && $("#sVinmStatus50").is(':checked') )  )){
                ...
                */
                if( e.sender.dataItem(e.sender.select()) != null ){
                    ...
                    if($("#sVinmStatus60").is(':checked')){ //미입고
 ...
 */
                    ...
                    **/
                    if($("#sVinmStatus60").is(':checked')){ //미입고
                        if(  dataItem.skipYn != "Y" && dataItem.statCd != 'A' ){
                            $(row).css("background-color", "#E0E0E0");
                            $(".grid-checkbox-item", row).each(function(){
                                $(this).prop("disabled" ,"disabled");
                            });
                        ...
                        if(  (dms.string.trim(dataItem.contractNo) != "NONE" ) ){
                            $(row).css("background-color", "#E0E0E0");
                            $(".grid-checkbox-item", row).each(function(){
                                $(this).prop("disabled" ,"disabled");
                            });
            ...
            ,columns:[
                ...
                ,{field:"skipYn"     ,width:100 ,attributes:{"class":"ac"} , hidden:true }  // 매핑스킵
                ,{field:"contractNo"  ,width:70  ,hidden:true}  // VIN_NO1
    ...
    $("#grid").on("click", ".checkAll", function(e){
          ...
          if(checked){
              rows.each(function(index, row) {
                  ...
                  //입고
                  console.log("sVinmStatus50="+$("#sVinmStatus50").is(':checked'));
                  if($("#sVinmStatus50").is(':checked')){
                      ...
                      if(dataItem[index].skipYn == "Y" || (dataItem[index].statCd == 'A') ){
                          $(".grid-checkbox-item[data-uid=" + dataItem[index].uid + "]").prop("checked", true);
                      }else{
                          $(".grid-checkbox-item[data-uid=" + dataItem[index].uid + "]").prop("checked", false);
                          chkRows.removeClass("k-state-selected");

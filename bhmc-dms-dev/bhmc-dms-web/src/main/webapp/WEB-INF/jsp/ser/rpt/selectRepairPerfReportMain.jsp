...
<section class="group" style="height:500%" >
    <div class="header_area">
        <h1 class="title_basic"><spring:message code="ser.title.rpirPerfReport" /></h1>  <!-- 수리실적 리포트 -->
    </div>
    ...
    <div id="tabstrip" class="tab_area tab_iframe">
        <!-- 탭메뉴 -->
        <ul>
            <li id="tab01" class="k-state-active"><spring:message code="ser.lbl.rpirPerf" /></li>              <!-- 수리실적 -->
            <li id="tab02"><spring:message code="ser.lbl.timeSearch" /></li>        <!-- 시간조회 -->
            <li id="tab03"><spring:message code="ser.lbl.fosterTp" /></li>          <!-- 차종명 -->
            <li id="tab04"><spring:message code="ser.lbl.carlineNm" /></li>         <!-- 위탁유형 -->
            <%-- <li id="tab05"><spring:message code="ser.lbl.rpirTp" /></li>       <!-- 수리유형 --> --%>
        </ul>
        <!-- //탭메뉴 -->
        <!-- 자체정의유형 -->
        <div class="mt0">
...
selectTab = function(tabId){
    ...
    if (tabId == "tab01") {
        if($("#iframeStatus01").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectRepairPerfRpirPerfReport.do' />";
            $("#iframeStatus01").attr("src", iframeUrl);
        }
    } else if (tabId == "tab02") {
        if($("#iframeStatus02").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectRepairPerfTimeSearchReport.do' />";
            $("#iframeStatus02").attr("src", iframeUrl);
        }
    } else if (tabId == "tab03") {
        if($("#iframeStatus03").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectRepairPerfFosterTpReport.do' />";
            $("#iframeStatus03").attr("src", iframeUrl);
        }
    } else if (tabId == "tab04") {
        if($("#iframeStatus04").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectRepairPerfCarLineNmReport.do' />";
            $("#iframeStatus04").attr("src", iframeUrl);
        }
    }  else if (tabId == "tab05") {
        if($("#iframeStatus05").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectRepairPerfRepairTpReport.do' />";

...
<section class="group" style="height:500%" >
    <div class="header_area">
        <h1 class="title_basic"><spring:message code="ser.title.supportPerfReport" /></h1>  <!-- 접대실적 리포트 -->
    </div>
    ...
    <div id="tabstrip" class="tab_area tab_iframe">
        <!-- 탭메뉴 -->
        <ul>
            <li id="tab01" class="k-state-active"><spring:message code="ser.lbl.supportPerf" /></li>              <!-- 접대실적 -->
            <li id="tab02"><spring:message code="ser.lbl.timeSearch" /></li>                <!-- 시간조회 -->
            <li id="tab03"><spring:message code="ser.lbl.fosterTp" /></li>                  <!-- 위탁유형 -->
            <li id="tab04"><spring:message code="ser.lbl.carlineNm" /></li>                 <!-- 차종명 -->
...
selectTab = function(tabId){
    ...
    if (tabId == "tab01") {
        if($("#iframeStatus01").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectSupportPerfSupportPerfReport.do' />";
            $("#iframeStatus01").attr("src", iframeUrl);
        }
    } else if (tabId == "tab02") {
        if($("#iframeStatus02").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectSupportPerfTimeSearchReport.do' />";
            $("#iframeStatus02").attr("src", iframeUrl);
        }
    } else if (tabId == "tab03") {
        if($("#iframeStatus03").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectSupportPerfFosterTpReport.do' />";
            $("#iframeStatus03").attr("src", iframeUrl);
        }
    } else if (tabId == "tab04") {
        if($("#iframeStatus04").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectSupportPerfCarLineNmReport.do' />";
            $("#iframeStatus04").attr("src", iframeUrl);
        }
    } /* else if (tabId == "tab05") {
        if($("#iframeStatus05").attr("src") == ""){
            iframeUrl = "<c:url value='/ser/rpt/selectSupportPerfRepairTpReport.do' />";

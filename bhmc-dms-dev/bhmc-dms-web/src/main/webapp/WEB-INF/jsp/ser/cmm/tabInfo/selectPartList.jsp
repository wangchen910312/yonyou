...
$(document).ready(function(){
    ...
    var reHiddenColYn = (preFixId == "RE" || preFixId == "PRE" || preFixId == "PC") ? true : false;
    ...
    $("#partGrid").kendoExtGrid({
        ...
        ,edit:function(e){
            ...
            if(dms.string.isNotEmpty(selectedVal.comItemPrc)){
                if(fieldName2 == "comItemNm"){
                    this.closeCell();
                }
        ...
        ,columns:[
            ...
            ,{field:"parReadyStatCdNm", title:"<spring:message code='serl.lbl.parReadyStatCd' />", width:80
             ...
             //活动代码
             ,{field:"lbrCd", title:"<spring:message code='ser.lbl.lbrCd' />", width:80
                 ,attributes:{"class":"ac"}
                 ,hidden:true
             } 
    ...
    $("#parDel").kendoButton({
        click:function(e) {
            ...
            ,removeRowUid = []
            ,length = partGrid.items().length
            ,items = partGrid.dataSource.data();
            ...
            rows.each(function(index, row) {
                ...
                if(dataItem.wrkStatCd == "02" || dataItem.wrkStatCd == "05"){
                    ...
                    var mainItemCrNoIsEmpty;
                    if(dms.string.isNotEmpty(dataItem.comItemPrc)){
                        for (var i = 0; i < items.length; i++) {
                            if((items[i].comItemCd === dataItem.itemCd)&&(items[i].uid != dataItem.uid)){
                                if(dms.string.isEmpty(items[i].crNo)){
                                    mainItemCrNoIsEmpty = true;
                                }else{
                                    mainItemCrNoIsEmpty = false;
                                }
                            }
                        }
                    }else{
                    ...
                    if(mainItemCrNoIsEmpty){
                        if(dms.string.isNotEmpty(dataItem.comItemPrc)){
                            removeRowUid.push(dataItem.uid);
                            for (var i = 0; i < items.length; i++) {
                                if((items[i].comItemCd === dataItem.itemCd)&&(items[i].uid != dataItem.uid)){
                                    removeRowUid.push(items[i].uid);
                                }
                            }
                        }else if(dms.string.isNotEmpty(dataItem.comItemCd)){
                            removeRowUid.push(dataItem.uid);
                            for (var i = 0; i < items.length; i++) {
                                if((items[i].itemCd == dataItem.comItemCd)&&(items[i].uid != dataItem.uid)){
                                    removeRowUid.push(items[i].uid);
                                }
                            }
                        }else{
                        ...
                        if(removeRowUid.length > 0){
                            for(var j = 0; j < removeRowUid.length; j++){
                              for (var i = 0; i < items.length; i++) {
                                  if(items[i].uid === removeRowUid[j]){
                                      var item = items[i];
                                      partGrid.dataSource.remove(item);
                                  }
                              }
                            }
                        }
    ...
    $("#parAdd").kendoButton({
        click:function(e) {
            ...
            if(preFixId == "RE" || preFixId == "PRE" || preFixId == "RO"){
             ...
             // R19030702012 客户反馈维修估算单中，添加库存为0的配件项目时，查询不到，实际情况即便库存是0，也是需要添加的，配件信息查询中看到该配件是在有效期内的。2019-3-13 贾明 start 
            if(preFixId == "ET" ){
                avlbStockQtyZeroYn = "N";
            }
            // R19030702012 客户反馈维修估算单中，添加库存为0的配件项目时，查询不到，实际情况即便库存是0，也是需要添加的，配件信息查询中看到该配件是在有效期内的。2019-3-13 贾明 end 
    ...
    partsAdd = function(jsonObj, campaignYn){
        ...
        $.each(jsonObj, function(idx, data){
            ...
            $.each(partDataItem, function(idx, obj){
               ...
               // 2020.10.26 tjx修改不同活动类型的配件不能叠加,需要覆盖
                if(campaignYn == "Y"){
               		if(data.crNo == obj.crNo && data.lbrCd != obj.lbrCd){
               			obj.lbrCd = "remove";
               		}
            	}
            });
            ...
            if(preFixId == "RE" || preFixId == "PRE"){   // 예약접수, 부품예약
                ...
                roTpCd = ;   tianjx 20190924 维修估算根据工单类型选择配件价格
            }*/else {
...
//                    }
                    ...
                    itemPrc = data.incPrc;  // 보험가
                    itemPrcTp = "03";
                } else {
                    itemPrc = data.retlPrc;
                    itemPrcTp = "01";
                }
				itemTotAmt = Number(itemPrc*((pkgYN == "Y") ?  data.itemQty:data.giQty));
            ...
            if(checkVal) {
                copyData ={
                    ...
                    //,itemPrc:(dlrCampaignYn || bhmcCampaignYn) ? data.itemPrc : itemPrc
                  ...
                  //,itemSaleAmt:(dlrCampaignYn || bhmcCampaignYn) ? data.itemTotAmt : (pkgYN == "Y") ? data.pkgDetlTotAmt : itemSaleAmt
                  	,itemTotAmt:(dlrCampaignYn || bhmcCampaignYn) ? data.itemTotAmt : itemTotAmt
                    ,itemSaleAmt:(dlrCampaignYn || bhmcCampaignYn) ? data.itemTotAmt : itemTotAmt
                    ...
                    ,reqQty:(pkgYN == "Y") ? data.itemQty : data.giQty
                    ,paymTp:paymTp
                    ,paymCd:paymCd
                    ,paymUsrNm:paymUsrNm
        ...
        //tjx 更换批量活动对应的配件需要替换,不是累加
        var partLength = partDataItem.length -1 ;
        for (var i = partLength ;i >= 0 ;i--){
        	if(partDataItem[i].lbrCd == "remove"){
        		partGrid.dataSource.remove(partDataItem[i]);
          	}
        }
    };
                        ...
                        if(parData.length > 0)
                        {
                            ...
                            rows.each(function(index, row) {
                                if($(this).hasClass("k-state-selected")){
                                    ...
                                    if(dataItem[index].itemCd == parData[0].itemCd){
                                        duplicateItemYn = true;
                                        return false;
                                    }

                                    preComItemCd = dataItem[index].comItemCd;
                                    var itemPrc = parData[0].retlPrc.toFixed(2);
                                    comItemPrc = Number(itemPrc*parData[0].giQty) - Number(dataItem[index].itemTotAmt)  <= 0 ? 0 : Number(itemPrc*parData[0].giQty) - Number(dataItem[index].itemTotAmt);
                            ...
                            if(duplicateItemYn){
                                dms.notification.info("<spring:message code='par.info.duplicateSelection'  />");
                                return false;
                            }
                            partsAdd(partData, "N", "Y");
                            var dataItem = partGrid.dataSource._data;
        ...
        if(chkRsultYn){
            $.each(getLbrData(), function(i , gridData){
                if(roTp == '02'){ //보증
                    ...
                    var lbrTp = (gridData.lbrTp == "03" || gridData.lbrTp == "04" ) ? gridData.lbrTp : "03" ;
        ...
        insurancePopup = dms.window.popup({
            ...
            ,content:{
                ...
                ,data:{
                    "autoBind":false
                    ,"selectable":"single"
                    ,"isTablet":isTablet //增加是否是pad端检验贾明 2018-11-14 
  ...
  //부품지불자 변경 이벤트
    chagePayTpInParts = function(grid){
        var params = {};
        var list = [];
        var payInfoList = [];
        ...
        $.ajax({
            url:"<c:url value='/par/pmm/partsPriceMaster/selectPayTpPriceMastersByKeys.do' />"
            ...
            ,success:function(result){
                if(result.data.length == gridData.length){
                    $.each(result.data, function(idx,row){

                        gridData[idx].set("itemPrc", row.salePrcAmt);
                        gridData[idx].set("itemSalePrc", row.saleTaxDdctPrcAmt);
                        gridData[idx].set("itemSaleAmt", Number(row.salePrcAmt*gridData[idx].reqQty));
                        gridData[idx].set("paymTp", payInfoList[idx].paymTp);
                        gridData[idx].set("paymCd", payInfoList[idx].paymCd);
                        gridData[idx].set("paymUsrNm", payInfoList[idx].paymUsrNm);
                        gridData[idx].set("partsPrcTp", payInfoList[idx].itemPrcTp);

                        // 할인내용이 있는상황에서 금액이 바뀌는 경우 가있어서 0으로 리셋
                        gridData[idx].set("dcRate", 0);
                        gridData[idx].set("dcAmt", 0);

                        setPartAmt(gridData[idx],"Amt");
    ...
    setPartAmt = function(rowData , flag){
        ...
        if(rowData != null){
            ...
            // var itemPrc = (preFixId == "ET" && roTp != "02") ? Number(rowData.itemSalePrc) : Number(rowData.itemPrc); tianjix 保修索赔单在估算时使用单价,其他单使用保证价

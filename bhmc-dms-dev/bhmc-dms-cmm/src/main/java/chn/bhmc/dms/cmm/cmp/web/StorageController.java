...
public class StorageController extends HController {
    ...
    public String selectStorageMain2(Model model) throws Exception {
        ...
        model.addAttribute("sysStartDate", DateUtil.getDate(new Date(), dateFormat));
    ...
    @ResponseBody
    public int deleteStorage(@RequestBody StorageVO storageVO)  throws Exception {

        storageVO.setRegUsrId(LoginUtil.getUserId());
        storageVO.setDlrCd(LoginUtil.getDlrCd());
        storageVO.setPltCd(LoginUtil.getPltCd());

        int result = storageService.deleteStorage(storageVO);
